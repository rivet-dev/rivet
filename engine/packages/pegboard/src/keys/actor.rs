...
#[derive(Debug)]
pub struct ActiveGenerationKey {
	actor_id: Id,
}

impl ActiveGenerationKey {
	pub fn new(actor_id: Id) -> Self {
		ActiveGenerationKey { actor_id }
	}
}

impl FormalKey for ActiveGenerationKey {
	type Value = u32;
...
impl TuplePack for ActiveGenerationKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
	) -> std::io::Result<VersionstampOffset> {
		let t = (ACTOR, DATA, self.actor_id, ACTIVE_GENERATION);
		t.pack(w, tuple_depth)
	}
}

impl<'de> TupleUnpack<'de> for ActiveGenerationKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (input, (_, _, actor_id, _)) = <(usize, usize, Id, usize)>::unpack(input, tuple_depth)?;

		let v = ActiveGenerationKey { actor_id };
