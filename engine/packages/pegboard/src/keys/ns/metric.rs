...
impl Metric {
	fn variant(&self) -> MetricVariant {
		match self {
			...
			Metric::GatewayIngress(_) => MetricVariant::GatewayIngress,
			Metric::GatewayEgress(_) => MetricVariant::GatewayEgress,
...
impl TuplePack for MetricKey {
	fn pack<W: std::io::Write>(
		...
		offset += match &self.metric {
			...
			Metric::GatewayIngress(actor_name) => actor_name.pack(w, tuple_depth)?,
			Metric::GatewayEgress(actor_name) => actor_name.pack(w, tuple_depth)?,
		...
		let (input, v) = match variant {
			...
			MetricVariant::GatewayIngress => {
				...
				(
					...
					MetricKey {
						namespace_id,
						metric: Metric::GatewayIngress(actor_name),
					},
				)
			}
			MetricVariant::GatewayEgress => {
				...
				(
					...
					MetricKey {
						namespace_id,
						metric: Metric::GatewayEgress(actor_name),
					},
				)
			}
