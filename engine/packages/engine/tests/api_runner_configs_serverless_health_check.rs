...
fn serverless_health_check_returns_invalid_response_json_for_malformed_body() {
	run(ctx::TestOpts::new(1), |ctx| async move {
		...
		match response {
			api::public::ServerlessHealthCheckResponse::Failure {
				error:
					api::public::ServerlessMetadataError::InvalidResponseJson {
						invalid_response_json,
					},
			} => {
				assert_eq!(
					invalid_response_json.body,
					r#"{"runtime":"rivetkit","version":"1""#
				);
				assert!(
					!invalid_response_json.parse_error.is_empty(),
					"parse_error should not be empty"
				);
			}
			other => panic!("expected invalid_response_json failure, got {other:?}"),
		}
	});
}

#[test]
fn serverless_health_check_returns_invalid_envoy_protocol_version() {
	run(ctx::TestOpts::new(1), |ctx| async move {
		let (namespace, _) = setup_test_namespace(ctx.leader_dc()).await;
		...
		match response {
			api::public::ServerlessHealthCheckResponse::Failure {
				error:
					api::public::ServerlessMetadataError::InvalidEnvoyProtocolVersion {
						invalid_envoy_protocol_version,
					},
			} => {
				assert_eq!(invalid_envoy_protocol_version.version, invalid_version);
			}
			other => panic!("expected invalid_envoy_protocol_version failure, got {other:?}"),
		}
	});
}
