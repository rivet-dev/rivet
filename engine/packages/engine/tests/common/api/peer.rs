...
pub async fn build_namespaces_list_request(
	...
	Ok(client
		.get(format!("{}/namespaces", get_endpoint(port)))
		.query(&query))
}
...
pub async fn build_runner_configs_list_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.get(format!("{}/runner-configs", get_endpoint(port)))
		.query(&query))
}
...
pub async fn build_actors_list_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.get(format!("{}/actors", get_endpoint(port)))
		.query(&query))
}
...
pub async fn build_actors_create_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.post(format!("{}/actors", get_endpoint(port)))
		.query(&query)
		.json(&request))
}
...
pub async fn build_actors_delete_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.delete(format!("{}/actors/{}", get_endpoint(port), path.actor_id))
		.query(&query))
}
...
pub async fn build_actors_list_names_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.get(format!("{}/actors/names", get_endpoint(port)))
		.query(&query))
}
...
pub async fn build_runners_list_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.get(format!("{}/runners", get_endpoint(port)))
		.query(&query))
}
...
pub async fn build_runners_list_names_request(
	...
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.get(format!("{}/runners/names", get_endpoint(port)))
		.query(&query))
}
...
pub async fn build_bump_serverless_autoscaler_request(
	port: u16,
) -> Result<reqwest::RequestBuilder> {
	let client = rivet_pools::reqwest::client().await?;
	Ok(client.post(format!("{}/bump-serverless-autoscaler", get_endpoint(port))))
}

pub async fn bump_serverless_autoscaler(
	port: u16,
) -> Result<rivet_api_peer::internal::BumpServerlessAutoscalerResponse> {
	let request = build_bump_serverless_autoscaler_request(port).await?;
	let response = request.send().await?;
	parse_response(response).await
}
