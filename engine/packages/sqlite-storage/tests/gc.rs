...
async fn test_db() -> Result<universaldb::Database> {
	let path = Builder::new().prefix("sqlite-storage-gc-").tempdir()?.keep();
...
async fn branch_gc_pin_recomputes_from_current_counters_without_ratchet() -> Result<()> {
	...
	db.run(move |tx| async move {
		...
		tx.informal().set(&branches_bk_pin_key(branch_id), &[8; 16]);
		Ok(())
	})
	.await?;
	...
	db.run(move |tx| async move {
		...
		tx.informal()
			.set(&branches_bk_pin_key(branch_id), &VERSIONSTAMP_INFINITY);
		Ok(())
	})
	.await?;
	let pin = estimate_branch_gc_pin(&db, branch_id)
...
async fn branch_hot_gc_uses_vtx_floor_for_commits_vtx_and_delta() -> Result<()> {
	...
	assert_eq!(read_value(&db, branch_commit_key(branch_id, 3)).await?, None);
	assert_eq!(read_value(&db, branch_commit_key(branch_id, 4)).await?, None);
	assert!(read_value(&db, branch_commit_key(branch_id, 6)).await?.is_some());
	assert!(read_value(&db, branch_commit_key(branch_id, 8)).await?.is_some());
	assert_eq!(read_value(&db, branch_vtx_key(branch_id, [3; 16])).await?, None);
	assert_eq!(read_value(&db, branch_vtx_key(branch_id, [4; 16])).await?, None);
	assert!(read_value(&db, branch_vtx_key(branch_id, [6; 16])).await?.is_some());
	assert!(read_value(&db, branch_vtx_key(branch_id, [8; 16])).await?.is_some());
	assert_eq!(
