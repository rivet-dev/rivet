...
- **All actor state is per-branch.** Keys are `[0x02][actor_id][BR][branch_id]/<suffix>`. A branch tree per actor with COW page-level sharing via parent fall-through.
...
- **Branch records live under `[BRANCHES]/list/{branch_id}` with an FDB atomic-add refcount and `oldest_descendant_parent_txid` atomic-min.** GC reads these scalars instead of walking the descendant tree.
