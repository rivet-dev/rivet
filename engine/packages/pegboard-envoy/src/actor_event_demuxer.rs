...
struct Channel {
	tx: mpsc::UnboundedSender<protocol::EventWrapper>,
	handle: JoinHandle<()>,
	last_seen: Instant,
}
...
#[tracing::instrument(name="demuxer_channel", skip_all, fields(ray_id=?ctx.ray_id(), req_id=?ctx.req_id(), %envoy_key, ?actor_id))]
