...
async fn test_db() -> Result<universaldb::Database> {
	let path = Builder::new().prefix("depot-compact-").tempdir()?.keep();
...
async fn read_branch_compact_txid(
	...
	let bytes = read_value(db, depot::keys::branch_meta_compact_key(branch_id))
		.await?
...
async fn compact_sweeps_old_commits_even_without_selected_deltas() -> Result<()> {
	...
	seed(
		&db,
		vec![(
			depot::keys::branch_meta_compact_key(branch_id),
			encode_meta_compact(MetaCompact {
				materialized_txid: 1,
			})?,
		)],
