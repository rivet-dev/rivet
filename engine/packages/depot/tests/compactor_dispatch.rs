...
use depot::{
	...
	types::{DatabaseBranchId, DBHead, DirtyPage, MetaCompact, encode_db_head, encode_meta_compact},
};
...
fn test_ups() -> PubSub {
	PubSub::new(Arc::new(MemoryDriver::new(
		"depot-compactor-dispatch-test".to_string(),
	)))
}

async fn test_db() -> Result<universaldb::Database> {
	let path = Builder::new().prefix("depot-dispatch-").tempdir()?.keep();
...
async fn seed_compaction_case(db: &universaldb::Database, database_id: &str) -> Result<()> {
	let database_id = database_id.to_string();
	db.run(move |tx| {
		let database_id = database_id.clone();
		async move {
			tx.informal().set(
				&meta_head_key(&database_id),
			...
			tx.informal().set(
				&meta_compact_key(&database_id),
				&encode_meta_compact(MetaCompact {
					materialized_txid: 0,
				})?,
			);
			tx.informal()
				.set(&delta_chunk_key(&database_id, 1, 0), &encoded_blob(1, &[(1, 0x11)])?);
			tx.informal()
				.set(&pidx_delta_key(&database_id, 1), &1_u64.to_be_bytes());
			Ok(())
		}
	})
	.await
...
async fn steal_lease(db: &universaldb::Database, database_id: &str) -> Result<()> {
	let database_id = database_id.to_string();
	db.run(move |tx| {
		let database_id = database_id.clone();
		async move {
			tx.informal().set(
				&meta_compactor_lease_key(&database_id),
...
fn compact_payload_round_trips_with_embedded_version() {
	for payload in [
		...
		SqliteCompactPayload {
			...
			database_name: Some("database-a".to_string()),
...
async fn publish_compact_trigger_returns_unit_not_future() {
	let ups = test_ups();
	let _: () = publish_compact_trigger(&ups, "database-1");
}
...
async fn publish_compact_trigger_sends_fire_and_forget_ups_message() {
	...
	publish_compact_trigger(&ups, "database-a");
...
async fn ups_trigger_arrives_and_spawned_handler_compacts() -> Result<()> {
	...
	let ups = test_ups();
	let database_id = "database-worker-basic";
	seed_compaction_case(&db, database_id).await?;
	let mut sub = ups
		.queue_subscribe(SqliteCompactSubject, "compactor")
		.await
		.expect("subscriber should start");

	publish_compact_trigger(&ups, database_id);
	...
	assert!(
		read_value(&db, delta_chunk_key(database_id, 1, 0))
			.await?
			.is_none()
	);
	Ok(())
...
async fn lease_renewal_extends_local_deadline_mid_flight() -> Result<()> {
	...
	let db = Arc::new(test_db().await?);
	let database_id = "database-worker-renew";
	seed_compaction_case(&db, database_id).await?;
	let (_guard, reached, release) = test_hooks::pause_after_plan(database_id);
	let handle = tokio::spawn(worker::test_hooks::handle_trigger_once(
		Arc::clone(&db),
...
async fn lease_renewal_failure_cancels_compaction() -> Result<()> {
	let _pause_test_lock = PAUSE_TEST_LOCK.lock().await;
	let db = Arc::new(test_db().await?);
	let database_id = "database-worker-stolen";
	seed_compaction_case(&db, database_id).await?;
	let (_guard, reached, release) = test_hooks::pause_after_plan(database_id);
	let handle = tokio::spawn(worker::test_hooks::handle_trigger_once(
		Arc::clone(&db),
	...
	reached.notified().await;
	tokio::task::yield_now().await;
	steal_lease(&db, database_id).await?;
