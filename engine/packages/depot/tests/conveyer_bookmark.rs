...
async fn test_db() -> Result<universaldb::Database> {
	let path = Builder::new().prefix("depot-bookmark-").tempdir()?.keep();
...
fn test_ups() -> PubSub {
	PubSub::new(Arc::new(MemoryDriver::new(
		"depot-conveyer-bookmark-test".to_string(),
	)))
}
...
async fn namespace_branch_id(
	db: &universaldb::Database,
	namespace_id: NamespaceId,
) -> Result<depot::types::NamespaceBranchId> {
	db.run(move |tx| async move {
...
async fn create_pinned_bookmark_enforces_namespace_pin_cap() -> Result<()> {
	...
	db.run(move |tx| async move {
		tx.informal().set(
			...
			&i64::from(depot::constants::MAX_PINS_PER_NAMESPACE).to_le_bytes(),
		);
		Ok(())
	})
	.await?;
