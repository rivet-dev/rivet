...
async fn cold_compactor_lease_loss_b_to_c() -> Result<()> {
	let db = Arc::new(fault_common::test_db("depot-cold-lease-loss-").await?);
	fault_common::seed_cold_branch(&db).await?;
	let cold_root = Builder::new().prefix("depot-cold-lease-loss-").tempdir()?;
	...
	assert!(
		fault_common::read_value(
			&db,
			depot::keys::branch_meta_cold_lease_key(fault_common::database_branch_id())
		)
		.await?
		.is_none(),
