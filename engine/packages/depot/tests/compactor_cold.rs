...
fn test_ups() -> PubSub {
	PubSub::new(Arc::new(MemoryDriver::new(
		"depot-cold-compactor-test".to_string(),
	)))
}
...
fn page(pgno: u32, fill: u8) -> depot::types::DirtyPage {
...
async fn test_db() -> Result<universaldb::Database> {
	let path = Builder::new().prefix("depot-cold-compactor-").tempdir()?.keep();
...
) -> Result<Option<depot::compactor::cold::ColdCompactState>> {
...
) -> Result<(String, depot::compactor::cold::ColdPendingMarker)> {
	...
	db.run(move |tx| async move {
		tx.informal().set(
			...
			&encode_database_branch_record(DatabaseBranchRecord {
				branch_id,
				namespace_branch: depot::types::NamespaceBranchId::nil(),
				parent: None,
		...
		tx.informal()
			.set(&depot::keys::branch_delta_chunk_key(branch_id, 6, 0), b"delta-six");
		tx.informal().set(
		...
		tx.informal().set(
			&depot::keys::bookmark_pinned_key("database-a", bookmark().as_str()),
...
impl ColdTier for AdvancingColdDrainedTier {
	async fn put_object(&self, key: &str, bytes: &[u8]) -> Result<()> {
			...
			&& !self.advanced.swap(true, Ordering::SeqCst)
		{
			...
			db.run(|tx| async move {
				let state = depot::compactor::cold::ColdCompactState {
					cold_drained_txid: 4,
...
async fn cold_phase_b_uploads_layers_manifest_snapshot_and_cleans_stale_markers() -> Result<()> {
	...
	tier.put_object(
		&stale_marker_key,
		&encode_pending_marker(depot::compactor::cold::ColdPendingMarker {
			schema_version: depot::types::SQLITE_STORAGE_COLD_SCHEMA_VERSION,
