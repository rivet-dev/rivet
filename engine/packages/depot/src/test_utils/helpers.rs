//! Shared test helpers for depot integration tests.
...
pub async fn test_db_with_path() -> Result<(universaldb::Database, Subspace, PathBuf)> {
	let path = Builder::new().prefix("depot-").tempdir()?.keep();
	let db = open_test_db(&path).await?;
	let subspace = Subspace::new(&("depot", Uuid::new_v4().to_string()));
