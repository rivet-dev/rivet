...
impl ProtocolCompatibilityFeature {
	fn description(self, direction: ProtocolCompatibilityDirection) -> &'static str {
		match self {
			...
			ProtocolCompatibilityFeature::SqlitePreloadHints => match direction {
				ProtocolCompatibilityDirection::ToEnvoy => "sqlite preload hint responses",
				ProtocolCompatibilityDirection::ToRivet => "sqlite preload hint requests",
			},
...
impl fmt::Display for ProtocolCompatibilityError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		let verb = match self.feature {
			...
			| ProtocolCompatibilityFeature::SqlitePreloadHints
...
impl OwnedVersionedData for ToEnvoy {
	...
	fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
		...
		match version {
			1 => serde_bare::to_vec(&convert_to_envoy_v2_to_v1(convert_to_envoy_v3_to_v2(
				convert_to_envoy_v4_to_v3(convert_to_envoy_v5_to_v4(data, 1)?, 1)?,
			)?)?)
			.map_err(Into::into),
			2 => serde_bare::to_vec(&convert_to_envoy_v3_to_v2(convert_to_envoy_v4_to_v3(
				convert_to_envoy_v5_to_v4(data, 2)?,
				2,
			)?)?)
			.map_err(Into::into),
			3 => serde_bare::to_vec(&convert_to_envoy_v4_to_v3(
				convert_to_envoy_v5_to_v4(data, 3)?,
				3,
			)?)
			.map_err(Into::into),
			4 => serde_bare::to_vec(&convert_to_envoy_v5_to_v4(data, 4)?).map_err(Into::into),
...
impl OwnedVersionedData for ToRivet {
	...
	fn deserialize_version(payload: &[u8], version: u16) -> Result<Self> {
		Ok(Self::V5(match version {
			...
			3 => convert_to_rivet_v4_to_v5(convert_to_rivet_v3_to_v4(
	...
	fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
		...
		match version {
			1 | 2 => serde_bare::to_vec(&convert_to_rivet_v3_to_v2(convert_to_rivet_v4_to_v3(
				convert_to_rivet_v5_to_v4(data, version)?,
				version,
			)?)?)
			.map_err(Into::into),
			3 => serde_bare::to_vec(&convert_to_rivet_v4_to_v3(
				convert_to_rivet_v5_to_v4(data, 3)?,
				3,
			)?)
			.map_err(Into::into),
			4 => serde_bare::to_vec(&convert_to_rivet_v5_to_v4(data, 4)?).map_err(Into::into),
			5 => serde_bare::to_vec(&data).map_err(Into::into),
			_ => bail!("invalid version: {version}"),
		}
	}
...
impl OwnedVersionedData for ToEnvoyConn {
	...
	fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
		...
		match version {
			...
			4 => serde_bare::to_vec(&convert_same_bytes_ref::<_, v4::ToEnvoyConn>(&data)?)
				.map_err(Into::into),
			5 => serde_bare::to_vec(&data).map_err(Into::into),
			_ => bail!("invalid version: {version}"),
		}
...
impl OwnedVersionedData for ToGateway {
	...
	fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
		let Self::V5(data) = self;
		let data_v3 = || convert_same_bytes_ref::<_, v3::ToGateway>(&data);
		match version {
			1 => serde_bare::to_vec(&convert_to_gateway_v3_to_v1(data_v3()?)).map_err(Into::into),
			2 => serde_bare::to_vec(&convert_to_gateway_v3_to_v2(data_v3()?)).map_err(Into::into),
			3 => serde_bare::to_vec(&data_v3()?).map_err(Into::into),
			4 => serde_bare::to_vec(&convert_same_bytes_ref::<_, v4::ToGateway>(&data)?)
				.map_err(Into::into),
			5 => serde_bare::to_vec(&data).map_err(Into::into),
			_ => bail!("invalid version: {version}"),
		}
...
impl OwnedVersionedData for ToOutbound {
	...
	fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
		let Self::V5(data) = self;
		let data_v3 = || convert_same_bytes_ref::<_, v3::ToOutbound>(&data);
		match version {
			1 => serde_bare::to_vec(&convert_to_outbound_v3_to_v1(data_v3()?)).map_err(Into::into),
			2 => serde_bare::to_vec(&convert_to_outbound_v3_to_v2(data_v3()?)).map_err(Into::into),
			3 => serde_bare::to_vec(&data_v3()?).map_err(Into::into),
			4 => serde_bare::to_vec(&convert_same_bytes_ref::<_, v4::ToOutbound>(&data)?)
				.map_err(Into::into),
			5 => serde_bare::to_vec(&data).map_err(Into::into),
			_ => bail!("invalid version: {version}"),
		}
...
impl OwnedVersionedData for ActorCommandKeyData {
	...
	fn serialize_version(self, version: u16) -> Result<Vec<u8>> {
		...
		match version {
			...
			3 => serde_bare::to_vec(&data_v3()?).map_err(Into::into),
			4 => serde_bare::to_vec(&convert_same_bytes_ref::<_, v4::ActorCommandKeyData>(&data)?)
				.map_err(Into::into),
			5 => serde_bare::to_vec(&data).map_err(Into::into),
			_ => bail!("invalid version: {version}"),
		}
...
fn convert_to_envoy_v5_to_v4(message: v5::ToEnvoy, target_version: u16) -> Result<v4::ToEnvoy> {
	match &message {
		v5::ToEnvoy::ToEnvoySqlitePersistPreloadHintsResponse(_) => {
		...
		v5::ToEnvoy::ToEnvoyInit(_)
		| v5::ToEnvoy::ToEnvoyCommands(_)
		| v5::ToEnvoy::ToEnvoyAckEvents(_)
		| v5::ToEnvoy::ToEnvoyKvResponse(_)
		| v5::ToEnvoy::ToEnvoyTunnelMessage(_)
		| v5::ToEnvoy::ToEnvoyPing(_)
		| v5::ToEnvoy::ToEnvoySqliteGetPagesResponse(_)
		| v5::ToEnvoy::ToEnvoySqliteCommitResponse(_)
		| v5::ToEnvoy::ToEnvoySqliteExecResponse(_)
		| v5::ToEnvoy::ToEnvoySqliteExecuteResponse(_)
		| v5::ToEnvoy::ToEnvoySqliteExecuteWriteResponse(_) => {}
	}
...
fn convert_to_rivet_v5_to_v4(message: v5::ToRivet, target_version: u16) -> Result<v4::ToRivet> {
	match &message {
		v5::ToRivet::ToRivetSqlitePersistPreloadHintsRequest(_) => {
			return Err(incompatible(
				ProtocolCompatibilityFeature::SqlitePreloadHints,
		...
		v5::ToRivet::ToRivetMetadata(_)
		| v5::ToRivet::ToRivetEvents(_)
		| v5::ToRivet::ToRivetAckCommands(_)
		| v5::ToRivet::ToRivetStopping
		| v5::ToRivet::ToRivetPong(_)
		| v5::ToRivet::ToRivetKvRequest(_)
		| v5::ToRivet::ToRivetTunnelMessage(_)
		| v5::ToRivet::ToRivetSqliteGetPagesRequest(_)
		| v5::ToRivet::ToRivetSqliteCommitRequest(_)
		| v5::ToRivet::ToRivetSqliteExecRequest(_)
		| v5::ToRivet::ToRivetSqliteExecuteRequest(_)
		| v5::ToRivet::ToRivetSqliteExecuteWriteRequest(_) => {}
	}
