...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let schema_dir = workspace_root
		.join("sdks")
		.join("schemas")
		.join("kv-channel-protocol");

	// Rust type generation from BARE schemas.
	...
	if cli_js_path.exists() {
		typescript::generate_sdk(&schema_dir, workspace_root);
	} else {
		println!(
...
mod typescript {
	...
	pub fn generate_sdk(schema_dir: &Path, workspace_root: &Path) {
		...
		let highest_version_path = find_highest_version(schema_dir);
	...
	fn post_process_generated_ts(path: &Path) {
		...
		// Add PROTOCOL_VERSION constant at the top (not in the BARE schema).
		let content = format!("export const PROTOCOL_VERSION = 1;\n\n{content}");
