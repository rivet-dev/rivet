...
) -> Result<models::NamespacesCreateResponse, Error<NamespacesCreateError>> {
...
) -> Result<models::NamespaceListResponse, Error<NamespacesListError>> {
	// add a prefix to parameters to efficiently prevent name collisions
	let p_limit = limit;
	let p_cursor = cursor;
	let p_name = name;
	...
	if let Some(ref param_value) = p_namespace_id {
		req_builder = match "multi" {
			"multi" => req_builder.query(
				&param_value
					.into_iter()
					.map(|p| ("namespace_id".to_owned(), p.to_string()))
