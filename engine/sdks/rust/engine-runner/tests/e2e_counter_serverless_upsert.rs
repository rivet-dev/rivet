...
async fn counter_actor_serverless_upsert_config_http_kv_e2e() -> Result<()> {
	let _test_lock = common::acquire_test_lock().await?;
	let engine = common::EngineProcess::start().await?;
	let namespace = "default".to_string();

	let runner_name = common::random_name("rust-counter-serverless-upsert");
	...
	serverless_runner
		.upsert_serverless_runner_config(&serverless_url)
		.await
		.context("failed to upsert serverless runner config")?;
...
async fn get_count(State(ctx): State<ActorRequestContext>) -> Result<Json<Value>, StatusCode> {
	let count = ctx
		.kv_get_u64("count")
		.await
...
async fn increment(State(ctx): State<ActorRequestContext>) -> Result<Json<Value>, StatusCode> {
	let count = ctx
		.kv_get_u64("count")
		.await
		.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
