...
struct HttpRequestState {
	...
	_method: String,
	path: String,
	_headers: HashableMap<String, String>,
	body: Vec<u8>,
	stream: bool,
}

#[derive(Debug, Clone)]
enum TunnelRequestKind {
	Http(HttpRequestState),
	WebSocket,
}

#[derive(Debug, Clone)]
struct TunnelRequestState {
	gateway_id: protocol::GatewayId,
	request_id: protocol::RequestId,
	next_message_index: u16,
	kind: TunnelRequestKind,
}
...
struct InternalConfig {
	...
	default_actor_factory: Option<ActorFactory>,
}
...
impl EnvoyBuilder {
	...
	/// Register a fallback actor factory used when no actor-specific behavior is registered.
	pub fn with_default_actor_behavior<F>(mut self, factory: F) -> Self
	where
		F: Fn(ActorConfig) -> Box<dyn TestActor> + Send + Sync + 'static,
	{
		self.default_actor_factory = Some(Arc::new(factory));
		self
	}
	...
	pub fn build(self) -> Result<Envoy> {
		let config = InternalConfig {
			...
			default_actor_factory: self.default_actor_factory,
		};
		...
		Ok(Envoy {
			...
			lifecycle_tx,
			tunnel_requests: Arc::new(Mutex::new(HashMap::new())),
...
impl Envoy {
	...
	fn clone_for_task(&self) -> Self {
		Self {
			...
			tunnel_requests: self.tunnel_requests.clone(),
	...
	/// Send a sleep intent for the latest generation of an actor.
	pub async fn sleep_actor(&self, actor_id: &str) {
		let generation = {
			let actors = self.actors.lock().await;
			actors.get(actor_id).map(|actor| actor.generation)
		};
		...
		if let Some(generation) = generation {
			let _ = self.event_tx.send(ActorEvent {
				...
				generation,
				event: utils::make_actor_intent(protocol::ActorIntent::ActorIntentSleep),
			});
		}
	}

	/// Start a serverless actor from the payload passed to `/api/rivet/start`.
	pub async fn start_serverless_actor(&self, payload: &[u8]) -> Result<()> {
		if payload.len() < 2 {
			anyhow::bail!("serverless payload missing protocol version");
		}

		let version = u16::from_le_bytes([payload[0], payload[1]]);
		if version != PROTOCOL_VERSION {
			anyhow::bail!(
				"serverless payload protocol version mismatch: {} vs {}",
				version,
				PROTOCOL_VERSION
			);
		}

		let message = utils::decode_to_envoy(&payload[2..], version)?;
		let protocol::ToEnvoy::ToEnvoyCommands(commands) = message else {
			anyhow::bail!("invalid serverless payload");
		};
		if commands.len() != 1 {
			anyhow::bail!("invalid serverless payload");
		}

		let command = commands.into_iter().next().expect("checked single command");
		let checkpoint = command.checkpoint;
		let protocol::Command::CommandStartActor(start_cmd) = command.inner else {
			anyhow::bail!("invalid serverless payload");
		};
	...
	async fn handle_message(&self, ws_stream: &mut WsStream, buf: &[u8]) -> Result<()> {
		...
		match msg {
			...
			protocol::ToEnvoy::ToEnvoyTunnelMessage(message) => {
	...
	async fn handle_commands(&self, commands: Vec<protocol::CommandWrapper>, ws_stream: &mut WsStream) -> Result<()> {
	...
	async fn handle_start_actor(
		...
		let factory = self
			...
			.get(&cmd.config.name)
			.cloned()
			.or_else(|| self.config.default_actor_factory.clone())
			.context(format!(
	...
	async fn handle_tunnel_message(
		...
		match message.message_kind {
			protocol::ToEnvoyTunnelMessageKind::ToEnvoyRequestStart(req) => {
				self
					.handle_request_start(ws_stream, message.message_id, req)
					.await?;
			}
			protocol::ToEnvoyTunnelMessageKind::ToEnvoyRequestChunk(chunk) => {
				self
					.handle_request_chunk(ws_stream, message.message_id, chunk)
					.await?;
			}
			protocol::ToEnvoyTunnelMessageKind::ToEnvoyRequestAbort => {
				self.handle_request_abort(message.message_id).await;
			}
			protocol::ToEnvoyTunnelMessageKind::ToEnvoyWebSocketOpen(open) => {
				self
					.handle_websocket_open(ws_stream, message.message_id, open)
					.await?;
			}
			protocol::ToEnvoyTunnelMessageKind::ToEnvoyWebSocketMessage(msg) => {
				self
					.handle_websocket_message(ws_stream, message.message_id, msg)
					.await?;
			}
			protocol::ToEnvoyTunnelMessageKind::ToEnvoyWebSocketClose(close) => {
				self
					.handle_websocket_close(ws_stream, message.message_id, close)
					.await?;
			}
		}
	...
	async fn handle_request_start(
		...
		if !self.has_actor(&req.actor_id).await {
			self
				.send_unknown_actor_response(ws_stream, &message_id.gateway_id, &message_id.request_id)
				.await?;
			return Ok(());
		}

		let key = tunnel_key(&message_id.gateway_id, &message_id.request_id);
		let state = TunnelRequestState {
			gateway_id: message_id.gateway_id,
			request_id: message_id.request_id,
			next_message_index: 0,
			kind: TunnelRequestKind::Http(HttpRequestState {
		...
		let should_dispatch = matches!(&state.kind, TunnelRequestKind::Http(http) if !http.stream);

		self.tunnel_requests.lock().await.insert(key.clone(), state);

		if should_dispatch {
			self.dispatch_http_request(ws_stream, &key).await?;
		}
	...
	async fn handle_request_chunk(
		...
		{
			let mut requests = self.tunnel_requests.lock().await;
			if let Some(TunnelRequestState {
				kind: TunnelRequestKind::Http(state),
				..
			}) = requests.get_mut(&key)
			{
				state.body.extend(chunk.body);
				should_dispatch = chunk.finish;
			}
		}
	...
	async fn handle_request_abort(&self, message_id: protocol::MessageId) {
		let key = tunnel_key(&message_id.gateway_id, &message_id.request_id);
		self.tunnel_requests.lock().await.remove(&key);
	}

	async fn dispatch_http_request(&self, ws_stream: &mut WsStream, key: &str) -> Result<()> {
		let request = {
			let requests = self.tunnel_requests.lock().await;
			let Some(TunnelRequestState {
				kind: TunnelRequestKind::Http(state),
				..
			}) = requests.get(key)
			else {
		...
		let response = self.handle_http_request(request).await;
		self.send_http_response(ws_stream, key, response).await?;
		self.tunnel_requests.lock().await.remove(key);

		Ok(())
	}

	async fn handle_http_request(&self, request: HttpRequestState) -> protocol::ToRivetResponseStart {
		match request.path.as_str() {
			"/ping" => {
				...
				let headers = HashableMap::from_iter([
			...
			"/sleep" => {
				self.sleep_actor(&request.actor_id).await;
	...
	async fn send_http_response(
		...
		ws_stream: &mut WsStream,
		key: &str,
		response: protocol::ToRivetResponseStart,
	) -> Result<()> {
		self
			.send_tunnel_message(
				ws_stream,
	...
	async fn handle_websocket_open(
		...
		if !self.has_actor(&open.actor_id).await {
			self
				.send_tunnel_message_direct(
					ws_stream,
		...
		let key = tunnel_key(&message_id.gateway_id, &message_id.request_id);
		self.tunnel_requests.lock().await.insert(
			key.clone(),
			TunnelRequestState {
				gateway_id: message_id.gateway_id,
				request_id: message_id.request_id,
				next_message_index: 0,
				kind: TunnelRequestKind::WebSocket,
			},
		);
	...
	async fn handle_websocket_message(
		...
		let key = tunnel_key(&message_id.gateway_id, &message_id.request_id);
		let exists = self.tunnel_requests.lock().await.contains_key(&key);
		if !exists {
			return Ok(());
		}
	...
	async fn handle_websocket_close(
		...
		_ws_stream: &mut WsStream,
		message_id: protocol::MessageId,
		_close: protocol::ToEnvoyWebSocketClose,
	) -> Result<()> {
		let key = tunnel_key(&message_id.gateway_id, &message_id.request_id);
		self.tunnel_requests.lock().await.remove(&key);
		Ok(())
	...
	async fn send_unknown_actor_response(
		&self,
		ws_stream: &mut WsStream,
		gateway_id: &protocol::GatewayId,
		request_id: &protocol::RequestId,
		...
		let headers = HashableMap::from_iter([
			("x-rivet-error".to_string(), "envoy.actor_not_found".to_string()),
			("content-length".to_string(), body.len().to_string()),
		]);
	...
	async fn send_tunnel_message(
		...
		let (gateway_id, request_id, message_index) = {
			let mut requests = self.tunnel_requests.lock().await;
			let request = requests
				.get_mut(key)
				.context("missing tunnel request state")?;
			let message_index = request.next_message_index;
			request.next_message_index = request.next_message_index.wrapping_add(1);
			(request.gateway_id, request.request_id, message_index)
		};

		self
			.send_tunnel_message_direct(ws_stream, gateway_id, request_id, message_index, message_kind)
			.await
	}
	...
	async fn send_tunnel_message_direct(
		...
		let payload = protocol::ToRivet::ToRivetTunnelMessage(protocol::ToRivetTunnelMessage {
			message_id: protocol::MessageId {
				gateway_id,
				request_id,
				message_index,
		...
		let encoded = utils::encode_to_rivet(payload);
		ws_stream.send(Message::Binary(encoded.into())).await?;
		Ok(())
	}
...
fn tunnel_key(gateway_id: &protocol::GatewayId, request_id: &protocol::RequestId) -> String {
	format!("{}:{}", hex::encode(gateway_id), hex::encode(request_id))
}
