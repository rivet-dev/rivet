...
export class RivetClient {
		...
		if (actorId != null) {
			if (Array.isArray(actorId)) {
				_queryParams.actor_id = actorId.map((item) =>
					serializers.RivetId.jsonOrThrow(item, {
	...
	public async actorsKvGet(
		...
		if (_response.ok) {
			return serializers.ActorsKvGetResponse.parseOrThrow(
				_response.body,
				{
					unrecognizedObjectKeys: "passthrough",
					allowUnrecognizedUnionMembers: true,
	...
	public async actorsReschedule(
		...
		const _response = await (this._options.fetcher ?? core.fetcher)({
			...
			queryParameters: _queryParams,
			requestType: "json",
			body: serializers.ActorsRescheduleRequestBody.jsonOrThrow(_body, {
				unrecognizedObjectKeys: "strip",
			}),
			timeoutMs:
				requestOptions?.timeoutInSeconds != null
		...
		if (_response.ok) {
			return serializers.ActorsRescheduleResponse.parseOrThrow(
				_response.body,
				{
					unrecognizedObjectKeys: "passthrough",
					allowUnrecognizedUnionMembers: true,
		...
		switch (_response.error.reason) {
			...
			case "timeout":
				throw new errors.RivetTimeoutError(
					"Timeout exceeded when calling POST /actors/{actor_id}/reschedule.",
				);
			case "unknown":
				throw new errors.RivetError({
					message: _response.error.errorMessage,
	...
	public async actorsSleep(
		...
		const _response = await (this._options.fetcher ?? core.fetcher)({
			...
			queryParameters: _queryParams,
			requestType: "json",
			body: serializers.ActorsSleepRequestBody.jsonOrThrow(_body, {
				unrecognizedObjectKeys: "strip",
			}),
			timeoutMs:
				requestOptions?.timeoutInSeconds != null
		...
		if (_response.ok) {
			return serializers.ActorsSleepResponse.parseOrThrow(
				_response.body,
				{
					unrecognizedObjectKeys: "passthrough",
					allowUnrecognizedUnionMembers: true,
		...
		switch (_response.error.reason) {
			...
			case "timeout":
				throw new errors.RivetTimeoutError(
					"Timeout exceeded when calling POST /actors/{actor_id}/sleep.",
				);
			case "unknown":
				throw new errors.RivetError({
					message: _response.error.errorMessage,
	...
	public async runnerConfigsList(
		...
		const { namespace, limit, cursor, variant, runnerNames, runnerName } =
			request;
		const _queryParams: Record<
			string,
			string | string[] | object | object[] | null
		...
		if (variant != null) {
			_queryParams.variant = serializers.RunnerConfigVariant.jsonOrThrow(
				variant,
				{
					unrecognizedObjectKeys: "strip",
				},
