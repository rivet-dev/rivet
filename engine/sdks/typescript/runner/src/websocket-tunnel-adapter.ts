...
import { wrappingLteU16, wrappingAddU16, wrappingSubU16 } from "./utils";
...
export class WebSocketTunnelAdapter {
	...
	/**
	 * Called when a new message index is received for a HWS. This should
	 * persist the message index somewhere in order to restore the WebSocket.
	 *
	 * The receiver of this is in charge of sending Runner::sendHibernatableWebSocketMessageAck.
	 */
	#persistHibernatableWebSocketMessageIndex: (messageIndex: number) => void;
	...
	constructor(
		...
		this.#persistHibernatableWebSocketMessageIndex =
			persistHibernatableWebSocketMessageIndex;
	...
	_handleMessage(
		...
		if (this.#hibernatable) {
			this.#persistHibernatableWebSocketMessageIndex(messageIndex);
		}
	...
	_closeWithRetry(code?: number, reason?: string): void {
