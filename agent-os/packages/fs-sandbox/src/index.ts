...
/**
 * Check whether an error from the sandbox-agent SDK represents a "not found"
 * condition. Uses duck-typing because `instanceof SandboxAgentError` can fail
 * across ESM module boundaries.
 */
function isNotFound(err: unknown): boolean {
	...
	// The sandbox agent returns 400 with "path not found" for missing entries.
	if (status === 400) {
 ...
 */
export function createSandboxBackend(options: SandboxFsOptions): VirtualFileSystem {
	...
	const backend: VirtualFileSystem = {
		...
		async mkdir(p: string, options?: { recursive?: boolean }): Promise<void> {
			if (options?.recursive) {
				// Walk up and create each ancestor that doesn't exist.
				...
				// Non-recursive: check that the parent exists.
		...
		async removeDir(p: string): Promise<void> {
			// Check if the directory is non-empty before deleting.
			const entries = await client.listFsEntries({ path: resolve(p) });
