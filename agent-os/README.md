...
VM-level toolkits are available to all sessions:
...
### Passing toolkits to createSession()

Session-level toolkits are registered when a session is created. If a session toolkit has the same name as a VM-level toolkit, the session version overrides it:
...
- **`options.toolKits`** — `ToolKit[]` — Session-level toolkits (override VM-level on name collision).
