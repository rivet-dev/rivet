		...
		const initHandle = async () => {
			...
			switch (state.actorMethod) {
				case "get":
					...
					break;
				case "getOrCreate": {
					const createInput = state.createInput
						? JSON.parse(state.createInput)
						: undefined;
					baseHandle = accessor.getOrCreate(key, {
				...
				case "getForId":
					if (!state.actorId) {
						throw new Error(
							"Actor ID is required for getForId method",
						);
					}
				...
				case "create": {
					const createInput = state.createInput
						? JSON.parse(state.createInput)
						: undefined;
					baseHandle = await accessor.create(key, {
						input: createInput,
					});
					break;
				}
				default:
					throw new Error(
						`Unknown actor method: ${state.actorMethod}`,
					);
			}
