...
export default function ScheduleTab({ state, actorHandle }: TabProps) {
	...
	const handleScheduleAt = async () => {
		...
		try {
			...
			const result = await callAction("scheduleAlarmAt", [
				timestamp,
				data,
			]);
	...
	const handleScheduleAfter = async () => {
		...
		try {
			...
			const result = await callAction("scheduleAlarmAfter", [
				delay,
				data,
			]);
			setResponse(JSON.stringify(result, null, 2));
	...
	const handleGetHistory = async () => {
		...
		try {
			...
			setAlarmHistory((prev) =>
				prev.map((alarm) => ({
					...alarm,
					status: triggeredIds.includes(alarm.id)
						? "triggered"
						: alarm.status,
				})),
			);
	...
	if (state.actorName !== "demo") {
		return (
			<div className="section">
				<h3>Scheduling</h3>
				<p>
					Scheduling features are only available for the{" "}
						...
						<div style={{ marginBottom: "15px" }}>
							{alarmHistory.map((alarm) => (
								...
								>
									...
									<br />
									Scheduled for:{" "}
									{formatTime(alarm.scheduledFor)}
									{alarm.data &&
										Object.keys(alarm.data).length > 0 && (
											<>
												<br />
												Data:{" "}
