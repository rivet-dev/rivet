...
let PI_VER = pi.dependencies["@rivet-dev/agent-os-pi"];
// In agent-os local-dev mode (`agent-os-dep local`) the example's dep is rewritten
// to a `link:`/`file:`/`workspace:` value that plain `npm install` cannot consume,
// and the in-VM agent JS closure is not where the resume logic lives anyway (that
// is the Rust sidecar binary + napi). So for the closure fall back to a real
// published version. Override with AGENT_OS_CLOSURE_VERSION if needed.
if (/^(link:|file:|workspace:)/.test(PI_VER)) {
	PI_VER = process.env.AGENT_OS_CLOSURE_VERSION ?? "0.0.0-main.8794200";
	console.log(
		`[prepare-agent-modules] local link dep detected; using published closure version ${PI_VER}`,
	);
}
