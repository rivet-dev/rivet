...
const key = readFlagValue('--key') ?? `slow-reconnect-${timestampSlug()}-${randomSuffix()}`
...
try {
	...
	while (!stopping && (loop || wakeLoop <= wakeLoops)) {
		...
		for (let reconnectRun = 1; !stopping && reconnectRun <= runs; reconnectRun++) {
			try {
				const result = await runOnce(globalRun, wakeLoop, reconnectRun)
				printResult(globalRun, wakeLoop, reconnectRun, result)
			} catch (error) {
				console.error(`[wake ${wakeLoop} run ${reconnectRun}] failed:`, error)
				if (!loop) {
					throw error
				}
			}
