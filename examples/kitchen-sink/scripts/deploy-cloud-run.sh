...
# `kitchen-sink-staging` and `rivet-kitchen-sink` Cloud Run services in
# dev-projects-491221 / us-east4.
#
...
#
# Usage:
#   examples/kitchen-sink/scripts/deploy-cloud-run.sh [--only staging|prod]
#
# Defaults to deploying to BOTH services. Pass --only staging to deploy only
# kitchen-sink-staging, or --only prod for only rivet-kitchen-sink.
...
if [[ "${1:-}" == "--only" ]]; then
    ONLY="${2:-}"
    if [[ "$ONLY" != "staging" && "$ONLY" != "prod" ]]; then
        echo "error: --only must be staging or prod" >&2
        exit 1
    fi
fi
...
if [[ -z "$ONLY" || "$ONLY" == "staging" ]]; then
    deploy_one kitchen-sink-staging
fi
if [[ -z "$ONLY" || "$ONLY" == "prod" ]]; then
