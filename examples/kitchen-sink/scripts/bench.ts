...
async function measureBaseline(n = 20): Promise<number> {
...
function buildFailedEntry(group: string, name: string, err: unknown): BenchEntry {
	return {
		...
		perOpMs: null,
		failed: true,
		failReason: String(err).slice(0, 80),
	};
}
...
function benchChatLogInsert(): BenchFn[] {
	const group = "Chat Log Insert";
	const benches: BenchFn[] = [];

	for (const scenario of CHAT_LOG_SIZES) {
		const name = `Insert chat log (${scenario.label})`;
		if (!shouldRun(group, name)) continue;
		benches.push(async () => {
			const key = [`bench-chat-insert-${uid()}`];
			try {
				await callAction("testSqliteBench", key, "noop");
				const { result, ms: e2eMs } = await timed(() =>
...
function benchChatLogRead(): BenchFn[] {
	const group = "Chat Log Read";
	const benches: BenchFn[] = [];

	const readBenches = [
		{ suffix: "Select with limit", action: "chatLogSelectLimit" },
		{ suffix: "Select after index", action: "chatLogSelectIndexed" },
		{ suffix: "Count", action: "chatLogCount" },
		{ suffix: "Sum", action: "chatLogSum" },
	];

	for (const scenario of CHAT_LOG_SIZES) {
		for (const readBench of readBenches) {
			const name = `${readBench.suffix} (${scenario.label})`;
			if (!shouldRun(group, name)) continue;
			benches.push(async () => {
				const key = [`bench-chat-read-${uid()}`];
				try {
					await callAction("testSqliteBench", key, "noop");
					const { result, ms: e2eMs } = await timed(() =>
