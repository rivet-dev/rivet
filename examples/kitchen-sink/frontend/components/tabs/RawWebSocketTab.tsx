...
export default function RawWebSocketTab({ state }: TabProps) {
	...
	const sendMessage = () => {
		...
		try {
			if (isBinary) {
				...
				addMessage(
					"sent",
					`[Binary: ${bytes.length} bytes] ${hexString}`,
					true,
				);
			} else {
	...
	const exampleMessages = [
		{
			name: "Ping",
			data: '{"type": "ping", "timestamp": 0}',
			binary: false,
		},
		{
			name: "Echo",
			data: '{"type": "echo", "message": "Hello WebSocket!"}',
			binary: false,
		},
		...
		<div>
			...
			<div className="section">
				...
				>
					...
					{!isBinary && message.includes('"timestamp"') && (
						<button className="btn" onClick={updatePingTimestamp}>
							Update Timestamp
						</button>
					)}
				</div>
			...
			<div className="section">
				...
				{messages.length === 0 ? (
					...
					>
						No messages yet. Connect and send a message to see the
						conversation.
					</div>
				) : (
					<div className="event-list">
						{messages.map((msg) => (
							<div key={msg.id} className="event-item">
								...
								<span
									className={`name ${msg.type === "sent" ? "sent" : "received"}`}
								>
									{msg.type === "sent"
										? "→ SENT"
										: "← RECEIVED"}
