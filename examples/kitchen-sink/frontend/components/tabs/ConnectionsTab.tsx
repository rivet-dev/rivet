...
export default function ConnectionsTab({ state, actorHandle }: TabProps) {
	const [currentConnectionInfo, setCurrentConnectionInfo] =
		useState<any>(null);
	...
	if (state.actorName !== "demo") {
		return (
			<div className="section">
				<h3>Connections</h3>
				<p>
					Connection features are only available for the{" "}
				...
				<div className="section">
					...
					{!currentConnectionInfo ? (
						...
						>
							...
							<code>{currentConnectionInfo.connectionId}</code>

							<strong>Encoding:</strong>
							<span>
								{currentConnectionInfo.encoding.toUpperCase()}
							</span>
							...
							<span>
								{currentConnectionInfo.actorKey || "(empty)"}
							</span>

							{currentConnectionInfo.actorId && (
								<>
									<strong>Actor ID:</strong>
									<code>{currentConnectionInfo.actorId}</code>
								</>
							)}
							...
							<span>
								{formatTimestamp(
									currentConnectionInfo.lastActivity,
								)}
							</span>
						</div>
					)}
