...
export default defineConfig(({ mode }) => {
	...
	/** Baked into client bundle — use pk URL (RIVET_PUBLIC_ENDPOINT) on Render at build time. */
	const rivetPublic =
	...
	return {
		...
		resolve: {
			alias: {
				// UI stack imports `shiki` at bundle entry; stub avoids multi‑MB grammars (see `frontend/shiki-stub.ts`).
		...
		build: {
			outDir: "public",
			emptyOutDir: true,
			/** Large vendor chunks; threshold reduces noisy warnings (see `frontend/shiki-stub.ts`). */
