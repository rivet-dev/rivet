import { generateText, tool, stepCountIs, type Tool, type CoreMessage } from "ai";
...
export async function sendMessage(
	...
	try {
		...
		// Call generateText with the AI SDK
		...
		const result = await generateText({
			...
			onStepFinish: (step) => {
				console.log("[sendMessage] Step finished", { stepMessages: step.response?.messages?.length || 0 });
				options?.onStepFinish?.({
					response: { messages: step.response?.messages || [] },
				});
			},
		});
		console.log("[sendMessage] generateText completed", { textLength: result.text?.length || 0 });
