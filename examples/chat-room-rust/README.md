...
- **Actor Definition** ([`src/lib.rs`](https://github.com/rivet-dev/rivet/tree/main/examples/chat-room-rust/src/lib.rs)): Defines the `chatRoom` actor with SQLite-backed message history, persisted stats, `sendMessage` / `getHistory` / `getStats` actions, and a `newMessage` event
