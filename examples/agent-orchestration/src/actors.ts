...
const SYSTEM_PROMPT =
	"You are a focused orchestration assistant. Keep responses concise, actionable, and ready for handoff.";
...
export const agentManager = actor({
	...
	actions: {
		...
		createAgent: async (c, name?: string) => {
			...
			const agentName = trimmedName || `Agent ${c.state.agents.length + 1}`;
