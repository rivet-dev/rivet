// Reuses the same shared-engine helpers the @rivetkit/effect package tests use,
// so this example drives the actor through a real rivet-engine just like the
// SDK's own e2e suite (instead of per-suite auto-spawn, which can flake on a
// debug engine's health-check budget).
...
import {
	...
	TEST_ENGINE_TOKEN,
} from "../../../rivetkit-typescript/packages/rivetkit/tests/shared-engine.ts";
...
export async function prepareNamespace(
	...
	const namespace = options.namespace ?? `agent-effect-${randomUUID()}`;
...
async function createNamespace(
	...
	const response = await fetch(`${endpoint}/namespaces`, {
		...
		body: JSON.stringify({
			name: namespace,
			display_name: `Agent effect ${namespace}`,
		}),
	});
...
async function upsertNormalRunnerConfig(
	...
	while (Date.now() < deadline) {
		...
		// The engine briefly reports the just-created namespace as missing or
		// returns a transient internal_error before the create write
		// propagates. Retry both.
		if (
			(response.status === 400 &&
