				...
				>
					<div>
						<div className="idle-resources">
							<div className="idle-resources-label">
								Resources
							</div>
							<div className="idle-resources-value">
								{Math.floor(state.resources)}
							</div>
							<div className="idle-resources-total">
								Total produced:{" "}
								{Math.floor(state.totalProduced)}
							</div>
						</div>

						<div className="idle-buildings-header">
						...
						<div className="idle-buildings-grid">
							{state.buildings.map((b) => {
								...
								const progress = Math.min(
									1,
									elapsed / b.productionIntervalMs,
								);
								const remaining = Math.max(
									0,
									b.productionIntervalMs - elapsed,
								);
								return (
									<div
										key={b.id}
										className="idle-building-card"
									>
										<div className="idle-building-name">
											{b.name}
										</div>
										<div className="idle-building-rate">
											+{b.productionRate} /{" "}
											{b.productionIntervalMs / 1000}s
										</div>
						...
						<div
							className="idle-buildings-header"
							style={{ marginTop: 24 }}
						>
							Build New
						</div>
							...
							{BUILDINGS.map((bt) => (
								...
								>
									...
									<div className="idle-shop-rate">
										+{bt.productionRate} /{" "}
										{bt.productionIntervalMs / 1000}s
									</div>
								</button>
							))}
						</div>
					...
					<div>
						<div className="idle-leaderboard">
							<div className="idle-leaderboard-header">
								Leaderboard
							</div>
							{leaderboard.length === 0 ? (
								...
								leaderboard.map((entry, i) => (
									...
									>
										<span className="idle-leaderboard-rank">
											#{i + 1}
										</span>
										<span className="idle-leaderboard-name">
											{entry.playerName}
										</span>
										<span className="idle-leaderboard-score">
											{entry.totalProduced}
										</span>
									</div>
								))
