...
describe("matchmaking and session patterns", () => {
	test("io-style open lobby + 10 tps match with movement", async (ctx) => {
		...
		const firstResponse = (firstQueueResult as { response?: { matchId?: string; playerId?: string } })?.response;
		const secondResponse = (secondQueueResult as { response?: { matchId?: string; playerId?: string } })?.response;
		...
		const a = client.ioStyleMatch
			.getOrCreate([firstResponse!.matchId!], { params: { playerId: firstResponse!.playerId! } })
			.connect();
		const b = client.ioStyleMatch
			.getOrCreate([firstResponse!.matchId!], { params: { playerId: secondResponse!.playerId! } })
			.connect();
		await sleep(260);
		...
		const a = client.battleRoyaleMatch
			.get([r1!.matchId], {
				params: {
					...
					reservationToken: r1!.reservationToken,
				},
			})
			.connect();
		const b = client.battleRoyaleMatch
			.get([r2!.matchId], {
				params: {
					...
					reservationToken: r2!.reservationToken,
				},
			})
			.connect();
		await sleep(300);
