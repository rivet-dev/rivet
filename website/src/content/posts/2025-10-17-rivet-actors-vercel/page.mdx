...
- **Sleep When Idle, Near-Zero Cold Starts**: Actors automatically hibernate when idle and wake up quickly on demand. Only pay for active compute time.

- **Architected For Insane Scale**: Automatically scale from zero to millions of concurrent actors. Pay only for what you use with instant scaling and near-zero cold starts.
...
- **Scale to zero with near-zero cold starts**: Actors scale down when idle to eliminate costs, yet wake quickly on demand with hibernation preserving state for immediate activation.
...
- Collaborative documents ([example code](https://github.com/rivet-dev/rivet/tree/main/examples/collaborative-document))
- Chat rooms ([example code](https://github.com/rivet-dev/rivet/tree/main/examples/chat-room))
- Local-first sync ([example code](https://github.com/rivet-dev/rivet/tree/main/examples/state))
- Multiplayer games ([example code](https://github.com/rivet-dev/rivet/tree/main/examples/multiplayer-game))
- Agent streams ([example code](https://github.com/rivet-dev/rivet/tree/main/examples/ai-agent))
...
- **Do Rivet Actors have coldstarts?** They have near-zero cold starts. Vercel's platform and Rivet's hibernation model keep wake-up latency very low.
