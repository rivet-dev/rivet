...
import imgContainerVsIsolate from "./container-vs-isolate.svg";

Today, [Flue](https://github.com/rivet-dev/flue) officially supports [agentOS](https://agentos-sdk.dev).

Cloudflare Workers innovated by **running your code in a V8 isolate instead of a Docker container**. Most backends sit **idle waiting on a database**, an API, or the network, so booting a VM per request like AWS Lambda means paying for a lot of idle time.

Running backends this way unlocked two things:

- **Startup got cheap**: There's no container to boot per request the way Lambda does, so a Worker starts in milliseconds.
...
<img src={imgContainerVsIsolate.src} alt="A container held per request stays reserved through every idle stretch, while isolates in one process pack the same busy work with nothing reserved in between." />
...
So we applied the same model to the sandbox with agentOS by using **a WebAssembly and V8 isolate instead of a VM** for isolation. Against a best-in-class sandbox provider, agentOS is:
...
agentOS runs a **full Linux-compatible environment inside that WebAssembly isolate**. The agent gets a real shell, a filesystem, and languages like Bash, Node.js, and Python. That's a **real scripting layer**, not a chain of tool calls.
...
Instead of provisioning a heavy sandbox for every agent on the chance that one task needs it, agentOS **starts one on demand** and tears it down when the task finishes. The common path stays a 22 MB WebAssembly isolate, and you pay sandbox cost and sandbox cold starts only for the **small slice of work that actually requires one**.
...
So it **runs anywhere JavaScript does**, whether Node.js, Bun, or Deno, on platforms such as Rivet, Railway, Kubernetes, or Vercel. *(agentOS doesn't run on Cloudflare Workers yet, since it relies on OS-level APIs, but the same WebAssembly target we use to run agentOS in the browser experimentally makes it possible.)*
