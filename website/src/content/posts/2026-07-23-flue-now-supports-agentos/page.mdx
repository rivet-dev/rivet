...
Running backends with the isolate-based architecture unlocked two things:
...
It turns out, **agentic workloads look a lot like the backend workloads** that Cloudflare optimized with isolates. An agent **spends almost all of its time sitting idle** waiting on inference, tool calls, and the user to respond. But the sandbox an agent works in is a full VM, **booted and billed for the whole session** even while it sits idle.

So we applied the same model to the sandbox with agentOS by using **WebAssembly / V8 isolate instead of a VM** for isolation. Against a best-in-class sandbox provider, agentOS is:
...
![An agent's turn is mostly spent waiting: a traditional sandbox bills a full VM the entire turn, while agentOS bills only while computing](/images/blog/2026-07-23-flue-now-supports-agentos/sandbox-vs-isolate.svg)
