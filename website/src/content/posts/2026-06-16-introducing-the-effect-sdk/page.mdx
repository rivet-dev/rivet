...
description: "A first-class Effect SDK for Rivet Actors — your Scope, Ref, and Layer, now durable."
---
...
Effect maps onto that model cleanly. Its primitives normally live and die with a single process; on an actor they persist. If you know Effect, the whole thing fits in one line: an actor is a long-lived `Scope` holding a `Ref` that survives restarts. The SDK maps each primitive onto the actor without replacing it:
...
- **No mocks**: `Registry.test` swaps a real engine for an in-process one. The same actor code runs in tests.
...
Every cross-boundary call is serialized (CBOR), and the SDK places `Schema` at that boundary:

- **One schema, four uses**: an action's payload, success, and error, plus the persisted state.
...
### Concurrency, tracing, and logging that span actors

- **Concurrency across actors**: a coordinator calls many instances with `Effect.all`, and cancels them as one program.
- **Distributed spans**: `Effect.withSpan` in a handler nests under the SDK's server-side span, which nests under the client-side span.
- **Unified logs**: `Logger.layerPino` routes `Effect.log` output and the underlying RivetKit logs through one logger.

```ts
yield* Effect.all(
  rooms.map((room) => room.SendMessage({ sender: "Admin", text: "Deploying now" })),
  { concurrency: 16 },
).pipe(
  Effect.withSpan("broadcastAnnouncement", {
    attributes: { roomCount: rooms.length },
  }),
);
```
...
### Actions: typed RPC, even actor-to-actor

- **Schema-validated request/response**: end to end, as shown above.
- **Same client both directions**: the typed client works client-to-actor and actor-to-actor.
- **Errors compose**: a callee's typed error flows into the caller's declared error channel.
...
### SQLite: a database per actor

- **Embedded and per-instance**: add `db` to the actor; no connection strings.
- **Migrations run before wake**: handlers always see a migrated schema.
- **Local reads, scale to zero**: the database is co-located with the actor and idle when it sleeps.
...
### Events: realtime, no extra infrastructure

- **Broadcast from a handler**: every connected client receives it; no pub/sub service or separate WebSocket server.
- **Pairs with `State.changes`**: gives you a realtime data layer inside the actor.
...
### Queues: durable, ordered work

- **Durable and ordered**: actors carry completable queues for background and fire-and-forget work that outlives a request.
- **Maps to `forkScoped`**: a queue consumer is a fiber forked into the wake scope, so it lives as long as the actor is awake and tears down on sleep, the same pattern as `State.changes`.
- **Available today via the scheduler**: typed queue wrappers are landing through the beta; the scheduler is usable now through the actor context.
...
One actor that uses all of the above. The full runnable version is the [`chat-room-effect`](https://github.com/rivet-dev/rivet/tree/main/examples/chat-room-effect) example.

- **A service** for domain rules (`RoomPolicy`).
- **Persisted state** with a forked live change stream.
- **A sleep finalizer**.
- **Actor-to-actor RPC** whose typed error flows through this actor's channel.
- **SQLite** for history and **broadcast** for realtime.
...
[Effect AI](https://effect.website/docs/ai/introduction/) (shipped in `effect/unstable/ai`, with providers like `@effect/ai-openai`) gives you a provider-agnostic `LanguageModel` service. An actor gives that agent a durable home. Put together, you get an agent that remembers across calls and restarts, with the model swappable as a `Layer`. The full runnable version is the [`agent-effect`](https://github.com/rivet-dev/rivet/tree/main/examples/agent-effect) example.

Why the two fit:

- **Conversation memory is just state**: persist the message history in actor state, and the model sees every prior turn, even after a restart. No vector store or session cache to wire up for short-term memory.
- **The model is a `Layer`**: handlers require `LanguageModel` but never construct it, so you provide a real provider in production and a mock in tests without changing actor code.
- **One agent per conversation**: address an agent by key, single-writer, so concurrent messages to the same agent can't race on its history.
- **Typed inputs and errors**: guard the request with a `Schema.TaggedErrorClass`, the same as any other action.

The handler persists the user turn, calls the model with the running history, then persists the reply:
...
The actor stays provider-agnostic. The concrete model is a single `Layer<LanguageModel>` provided where the actor is composed:
...
export const OpenAiModelLayer = OpenAiLanguageModel.layer({
  ...
  Layer.provide(
    OpenAiClient.layerConfig({
      ...
      // Point at any OpenAI-compatible endpoint; the test aims this at a mock.
...
// In main.ts, provide it to the actor. Swapping providers is a one-line change.
const ActorsLayer = AgentLive.pipe(Layer.provide(OpenAiModelLayer));
```

Because the model is just a layer, the example's tests provide a `LanguageModel` aimed at an in-process mock LLM server (a real server, not a module mock) and run the agent end to end against a real engine, with no API key.

## Beta & Feature Support

The Effect SDK is in **beta**, and the `@rivetkit/effect` API may change between releases.

- **Typed today**: actions, state, the typed client, typed errors, logging, sleep, actor address, and the registry have typed, schema-validated APIs.
- **Available via raw context**: events and broadcast, schedule, embedded SQLite, queues, connections, and lifecycle hooks are usable now through the raw actor context while typed wrappers land.
- **Track it**: the [feature support matrix](/docs/actors/quickstart/effect#feature-support) lists the current state of each feature.

**The raw escape hatch.** Every wake function receives `rawRivetkitContext`, the underlying RivetKit [actor context](/docs/actors):

- It points at the same actor as the typed `state` argument, so you can mix both freely.
- Calls through it are not validated by `Schema`; payloads are typed as in the base RivetKit API. See [Raw Escape Hatch](/docs/actors/quickstart/effect#raw-escape-hatch).
...
This is the moment to shape the typed surface. If you build with it, tell us what you want wrapped next and what does or doesn't feel right in [Discord](https://rivet.dev/discord) or on [GitHub](https://github.com/rivet-dev/rivet). The matrix moves based on your feedback.
...
- Example projects: [`hello-world-effect`](https://github.com/rivet-dev/rivet/tree/main/examples/hello-world-effect), [`chat-room-effect`](https://github.com/rivet-dev/rivet/tree/main/examples/chat-room-effect), and [`agent-effect`](https://github.com/rivet-dev/rivet/tree/main/examples/agent-effect)
