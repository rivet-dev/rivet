...
keywords: ["rivet", "compute", "serverless", "deploy", "rivetkit", "hosting"]
title: "Introducing Rivet Compute"
description: "A serverless platform built for RivetKit apps. Deploy your backend to the cloud with a single command, no infrastructure to manage."
---

Today, as part of Launch Week, we're releasing **Rivet Compute**: a serverless platform dedicated to hosting RivetKit apps.

Until now, running a RivetKit backend meant bringing your own infrastructure. You either self-hosted the engine or deployed to a third-party platform like Vercel, and you owned the provisioning, scaling, and operations that came with it. Rivet Compute changes that. It's the first time you can deploy a RivetKit app directly onto infrastructure we run, with a single command and nothing to manage.
...
Rivet Compute isn't a generic container host that you have to adapt your app to. It's purpose-built for RivetKit, so the runtime, networking, and actor lifecycle all work the way your app expects out of the box.
