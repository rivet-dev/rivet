...
Unix gave humans a common language to control machines. Today we're releasing **[agentOS](/docs/agent-os)** to give agents the same power.
...
When you need a real Linux kernel for browsers, compilation, or desktop automation, agentOS can spin up a full sandbox on demand and [mount it into the VM](/docs/agent-os/sandbox) as a native filesystem directory. The agent reads and writes sandbox files the same way it reads local files.
...
- [agentOS documentation](/docs/agent-os)
- [Quickstart guide](/docs/agent-os/quickstart)
