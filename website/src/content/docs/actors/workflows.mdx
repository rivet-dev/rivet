...
When a workflow finishes normally, the actor sleeps until the next wake event. Workflow failures like exhausted step retries leave the workflow in a failed state and let the actor sleep. Only unhandled workflow errors surface as `run` errors.
