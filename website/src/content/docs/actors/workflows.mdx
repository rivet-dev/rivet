...
export const errorHookActor = actor({
  state: {
    lastError: null as WorkflowErrorState["lastError"],
  },
  run: workflow(
    async (ctx) => {
      await ctx.step({
    ...
    },
    {
      onError: (c, event) => {
        c.state.lastError = {
          type: event.type,
          stepName:
            event.type === "step" || event.type === "rollback"
              ? event.stepName
              : null,
          attempt: event.type === "step" ? event.attempt : null,
          willRetry: event.type === "step" ? event.willRetry : null,
          retryDelay: event.type === "step" ? event.retryDelay ?? null : null,
