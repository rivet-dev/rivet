...
When a workflow finishes normally, the actor sleeps until the next wake event. If a workflow throws, the actor stops with an error just like any other `run` handler.
