...
Create or update `next.config.ts` so Next.js keeps RivetKit's server runtime package external:
...
<CodeSnippet file="examples/docs/actors-quickstart-next-js/registry.ts" title="src/rivet/registry.ts" />
