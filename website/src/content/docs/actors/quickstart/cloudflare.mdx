...
Set up a Rivet project locally that runs on Cloudflare Workers. Use the public `@rivetkit/rivetkit-wasm` package, pass the bindings through `setup({ wasm })`, and use remote SQLite.
...
This example uses raw SQL to keep the runtime setup visible. With `runtime: "wasm"`, unset SQLite defaults to remote SQLite, and `sqlite: "local"` is rejected.
...
Your Worker is now serving the Rivet handler. The local Workers runtime must support outbound WebSockets for the Rivet envoy connection.
