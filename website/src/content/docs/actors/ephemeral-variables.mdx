...
const counter = actor({
  ...
  createVars: (c: InitContext, driver: any) => {
    return { 
...
<Note>
If accepting arguments to `createVars`, you **must** define the types: `createVars(c: InitContext, driver: any)`
...
const myActor = actor({
  ...
  createVars: (ctx: InitContext, driver: DriverContext) => ({ driver }),
