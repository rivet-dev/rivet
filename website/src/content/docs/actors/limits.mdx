...
| Max SQLite preload size | 768 KiB | — | Default maximum size of preloaded SQLite VFS data for RivetKit actors. Configurable per actor via `options.preloadMaxSqliteBytes`. Setting to 0 disables SQLite preloading for that actor. |
