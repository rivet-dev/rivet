...
Rivet automatically wraps your `onMigrate` callback in a transaction, so all schema changes are applied atomically in a single storage write. You do not need to add your own `BEGIN`/`COMMIT`.
