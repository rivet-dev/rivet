...
```bash
curl http://localhost:6420/gateway/{actor_id}/inspector/database/schema
```
...
```bash
curl "http://localhost:6420/gateway/{actor_id}/inspector/database/rows?table=test_data&limit=100&offset=0"
```
