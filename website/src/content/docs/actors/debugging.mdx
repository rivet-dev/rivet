...
#### Rerun Workflow From Step

Reset a workflow to a specific step and restart execution immediately. Omitting `entryId` reruns the workflow from the beginning.

```bash
curl -X POST http://localhost:6420/gateway/{actor_id}/inspector/workflow/rerun \
  -H 'Content-Type: application/json' \
