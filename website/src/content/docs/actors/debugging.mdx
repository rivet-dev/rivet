...
# Check the returned receipt later
curl $RIVET_API/gateway/{actor_id}/queue/receipts/{receipt_id}

# Forward an HTTP request to the actor's onRequest handler
curl $RIVET_API/gateway/{actor_id}/request/my/custom/path
```

Queue sends return a durable opaque receipt ID after acceptance:

```json
{ "receiptId": "...", "deduplicated": false }
```

Receipt lookups are nonblocking status snapshots:

```json
{ "state": "processing", "attempts": 1, "createdAt": 1784582400000, "startedAt": 1784582400100 }
```

Queue handlers do not return values to senders. Use an action or workflow when a caller needs a result.
