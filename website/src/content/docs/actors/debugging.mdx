...
| **Rivet Cloud** | Each actor auto-generates a unique inspector token stored in its KV. Retrieve it via the management KV endpoint (see below). Pass it as a bearer token in the `Authorization` header. |
