...
`c.queue.next()` throws `ActorAborted` when the actor is shutting down. Do not wrap `c.queue.next()` in `try/catch` inside `run` loops, or you may swallow shutdown and keep looping.

```typescript @nocheck
run: async (c) => {
  while (!c.aborted) {
    const msg = await c.queue.next("tasks", { timeout: 1000 });
    if (!msg) continue;

    // Catch processing errors separately if needed.
...
- `ActorAborted`: `c.queue.next()` interrupted because the actor is stopping.
