...
`createState(c: ActorContext, input: TInput): TState | Promise<TState>`
...
`onCreate(c: ActorContext, input: TInput): void | Promise<void>`
...
`onDestroy(c: ActorContext): void | Promise<void>`
...
`onWake(c: ActorContext): void | Promise<void>`
...
`onSleep(c: ActorContext): void | Promise<void>`
...
`createConnState(c: CreateConnStateContext, params: TConnParams): TConnState | Promise<TConnState>`
...
`onBeforeConnect(c: OnBeforeConnectContext, params: TConnParams): void | Promise<void>`
...
`onConnect(c: OnConnectContext, conn: Conn): void | Promise<void>`
...
`onDisconnect(c: ActorContext, conn: Conn): void | Promise<void>`
...
`onRequest(c: RequestContext, request: Request): Response | Promise<Response>`
...
`onWebSocket(c: ActorContext, websocket: WebSocket): void | Promise<void>`
...
`onBeforeActionResponse(c: ActorContext, name: string, args: unknown[], output: TOutput): TOutput | Promise<TOutput>`
...
```typescript
import { actor, InitContext } from "rivetkit";
...
const counter = actor({
  ...
  createState: (c: InitContext, input: CounterInput): CounterState => ({
