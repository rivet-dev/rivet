...
Actors do not need special handling for upgrades or eviction. The platform may force an actor to sleep during version upgrades or when a serverless request is about to timeout. The same [shutdown sequence](#sleep-shutdown-sequence) and timeouts apply, so `onSleep`, `waitUntil`, and abort signals all work as expected.
...
Requests that arrive during shutdown are held until the actor wakes up again. The caller does not need to retry.
