...
`setPreventSleep` does **not** prevent forced migrations. It only prevents the idle sleep timer from firing. Use `onSleep` and `waitUntil` to control cleanup before the actor moves to another machine.
...
The actor waits up to `waitUntilTimeout` (default 15 seconds) for all registered promises to settle during the [shutdown sequence](#shutdown-sequence). If the timeout is exceeded, the actor proceeds with sleep anyway.
...
4. `waitUntil` background promises are awaited (up to `waitUntilTimeout`). Nested `waitUntil` calls made inside earlier `waitUntil` callbacks are also drained within the same timeout window.
...
During steps 1 through 5, the actor is still fully functional. Database access, `broadcast`, `waitUntil`, `queue.send`, and `schedule.after` all work. State mutations are persisted at step 6. Actions invoked by pre-existing connections or lifecycle hooks continue to execute normally.
