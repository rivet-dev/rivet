...
- If the `run` handler throws an error, the actor will stop with an error
- On shutdown, the actor waits for the `run` handler to complete (with configurable timeout via `options.runStopTimeout`)

If you have a finite `run` handler and want the actor to be destroyed after it finishes its work, call `c.destroy()` explicitly before returning.
