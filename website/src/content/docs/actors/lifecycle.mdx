...
The entire window is bounded by `sleepGracePeriod` on sleep or `onDestroyTimeout` on destroy. Both default to 15 seconds. If the window is exceeded, the actor proceeds to state save anyway.
...
const myActor = actor({
  ...
  options: {
    ...
    // Total graceful destroy shutdown budget. Default: 15000ms.
    onDestroyTimeout: 15_000,
...
| `sleepGracePeriod` | 15000ms | Total graceful sleep shutdown window |
| `onDestroyTimeout` | 15000ms | Total graceful destroy shutdown window |
