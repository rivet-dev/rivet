...
Use queues to process durable messages with an `onMessage` callback. A successful return acknowledges the message; throwing retries it according to the queue's retry policy.
...
Use a `run` loop for advanced continuous background work inside an actor, such as custom raw queue consumption, streaming AI responses, or coordinating long-running tasks.
...
- **Queues** are durable and use at-least-once delivery with `onMessage`. Sends return receipts for status checks, never handler results; use an action or workflow when the caller needs a response.
