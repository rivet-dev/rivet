...
const userSession = actor({
  ...
  createVars: async (c): Promise<{ user: User }> => {
    ...
    const user = await db.users.findById(c.actorKey.join("-"));
    return { user };
  },
  ...
  actions: {
    ...
    updateEmail: async (c, email: string) => {
      c.state.requestCount++;
      await db.users.update(c.actorKey.join("-"), { email });
      // Refresh cached data
      c.vars.user = await db.users.findById(c.actorKey.join("-"));
    },
  },
});
...
const userActor = actor({
  ...
  onStateChange: async (c, newState) => {
    ...
    await db.users.update(c.actorKey.join("-"), {
