...
const chatRoom = actor({
  createState: (c: InitContext, input: ChatRoomInput) => ({
...
const game = actor({
  createState: (c: InitContext, inputRaw: z.infer<typeof GameInputSchema>) => {
    // Validate input
...
const game = actor({
  createState: (c: InitContext,  input: GameInput) => ({
    gameMode: input.gameMode,
    maxPlayers: input.maxPlayers,
    difficulty: input.difficulty ?? "medium",
  }),
