...
import { faNodeJs, faReact, faNextjs } from "@rivet-gg/icons";
...
const chatRoom = actor({
	...
	actions: {
		getRoomInfo: (c) => ({ org: c.actorKey[0], room: c.actorKey[1] }),
	},
});
...
// Access key inside actor via c.actorKey
```
...
const worker = actor({
	...
	run: workflow(async (ctx) => {
		await ctx.loop("task-loop", async (loopCtx) => {
			...
			await loopCtx.step("process-task", async (step) => {
...
const chatRoom = actor({
	...
	// Actor lifecycle
	onCreate: (c) => console.log("created", c.actorKey),
...
const client = c.client<typeof registry>();
await client.chatRoom.create([c.actorKey[0], name]);
...
const worker = actor({
	...
	run: workflow(async (ctx) => {
		await ctx.step("setup", async (step) => {
			...
			step.state.phase = "running";
			step.state.stopReason = null;
		});
		...
		const stopReason = await ctx.loop("worker-loop", async (loopCtx) => {
			...
			if (message.name === "work") {
				await loopCtx.step("apply-work", async (step) => {
					...
					step.state.processed += 1;
					step.state.total += message.body.amount;
				});
				return;
			}
		...
		await ctx.step("teardown", async (step) => {
			...
			step.state.phase = "stopped";
			step.state.stopReason = stopReason;
		});
	}),
});
