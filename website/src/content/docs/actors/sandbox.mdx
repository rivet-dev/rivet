...
import { sandboxDatabase } from "rivetkit/db";
...
## Preventing sleep during active turns

Set `preventSleepWhileTurnsActive` when the actor must stay awake while a
session turn is still producing events.
...
- Use `sandboxDatabase()` from `rivetkit/db` so the sandbox transcript tables are migrated automatically.
