...
| `rivet pool` | List the compute pools in a namespace. |
...
Builds and pushes your project's Docker image and upserts the managed pool, printing the dashboard URL. Deploys target the `default` pool unless you pass `--pool`; a new pool name is created on first deploy. See [Deploying to Rivet Compute](/docs/deploy/rivet-compute).
...
Lists the compute pools in a namespace. A namespace can hold multiple pools; `default` is the one used when a `deploy` or `logs` command omits `--pool`.

```bash
rivet pool list
```

Each line is printed as `<name>\t<status>`. Pass `--json` for machine-readable output.
