...
- **RocksDB only supports a single node.** Do not run multiple RocksDB nodes. Use the file system backend (RocksDB-based) for smaller single-node production deployments today. If you need distributed Rivet, contact [enterprise support](https://rivet.dev/sales) about FoundationDB.

## PostgreSQL

- **Treat PostgreSQL as experimental.** Validate the deployment carefully before rollout. If you need a production deployment today, use the file system backend (RocksDB-based) for smaller single-node deployments. If you need distributed Rivet, contact [enterprise support](https://rivet.dev/sales) about FoundationDB.
...
- **Use FoundationDB for distributed production deployments.** FoundationDB provides the best performance, scalability, and uptime for distributed Rivet. Contact [enterprise support](https://rivet.dev/sales) for FoundationDB guidance.
...
- **Contact [enterprise support](https://rivet.dev/sales) for distributed production deployments.** We can help with architecture review, scaling guidance, and FoundationDB support.
