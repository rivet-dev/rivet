...
- **RocksDB only supports a single node.** Do not run multiple RocksDB nodes. If you need horizontal scalability, PostgreSQL is currently experimental. For production on-prem Enterprise deployments, contact [enterprise support](https://rivet.dev/sales) about PostgreSQL or FoundationDB.

## PostgreSQL

- **Treat PostgreSQL as experimental.** Validate the deployment carefully before rollout. If you're deploying production on-prem for Enterprise, contact [enterprise support](https://rivet.dev/sales).
...
- **Contact [enterprise support](https://rivet.dev/sales) for production on-prem Enterprise deployments.** We can help with architecture review, scaling guidance, PostgreSQL evaluation, and FoundationDB support.
