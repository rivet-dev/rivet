...
<Steps>
  ...
  <Step title="Deploy">
    ...
    kubectl -n rivet-engine wait --for=condition=ready pod -l app=pgbouncer --timeout=300s
...
- Update the `postgres.url` connection string in `02-engine-configmap.yaml` to point to your managed instance (or to PgBouncer if you deploy it separately)
- Delete the bundled PostgreSQL manifests:
  ...
  - `15-pgbouncer-configmap.yaml`
  - `16-pgbouncer-deployment.yaml`
  - `17-pgbouncer-service.yaml`

  Most managed PostgreSQL services include built-in connection pooling (e.g. RDS Proxy, Cloud SQL connection pooler), so PgBouncer is typically not needed when using a managed service.
