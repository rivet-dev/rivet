...
The generated Flue server merges these actors into the same registry as the Flue agent and workflow actors.
...
`rivet()` accepts an optional `actors` module path. That module must export an `actors` object or a default actor map; the target merges it into the generated registry.

`agentOSSandbox()` accepts:

| Option | Required | Description |
| --- | --- | --- |
| `actor` | Yes | agentOS actor name from the exported actor map, such as `vm`. |
| `registry` | Yes | Use `flueRegistry` so the adapter shares the generated target registry. |
