...
|-|---|---|
| Persistence | None (in-memory only) | Persistent filesystem and sessions |
...
- **No persistent filesystem.** Files are lost when you call `dispose()`. You must configure your own mounts or persistence layer.
