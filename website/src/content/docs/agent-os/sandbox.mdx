...
// Each actor instance gets its own Docker sandbox via createOptions.
// The sandbox ID is persisted in state so the actor reconnects to the
// same container after a sleep/wake cycle instead of creating a new one.
const vm = agentOs({
