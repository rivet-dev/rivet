...
let unsubscribe = await conn.on("count") { (count: Int) in
    print(count)
}
...
let unsubscribe = await conn.on("message") { (from: String, body: String) in
    print("\(from): \(body)")
}
let unsubscribeOnce = await conn.once("gameOver") {
    print("done")
}
await unsubscribe()
...
Event callbacks support positional overloads for 0–5 args. If you need raw values or more than 5 arguments, use `JSONValue`:

```swift
let unsubscribe = await conn.on("message") { (args: [JSONValue]) in
    print(args)
}
```
