...
- **Ensure queue handlers are idempotent.** `onMessage` uses at-least-once delivery, so a handler may run again after a timeout or interrupted ack. See [Queues](/docs/actors/queues).
