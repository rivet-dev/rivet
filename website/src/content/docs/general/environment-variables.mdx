...
| Environment Variable | Description |
|---------------------|-------------|
| `RIVET_RUN_ENGINE` | Set to `1` to spawn the engine process or `0` to disable spawning. In development, RivetKit spawns the engine by default when no engine endpoint is configured. |
