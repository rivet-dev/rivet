...
The public endpoint is only required if using the [serverless runtime mode](/docs/general/runtime-modes#serverless-mode) and if you have a frontend using RivetKit.
...
This flow applies to [serverless runtime mode](/docs/general/runtime-modes#serverless-mode). For [envoy mode](/docs/general/runtime-modes#envoy-mode) or [clients configured to connect directly to Rivet](/docs/clients/javascript), clients connect directly to Rivet and this metadata flow is not needed.
...
|---------------------|---------------|-------------|
| `RIVET_ENDPOINT` | `engine.endpoint` | Rivet Engine URL for your backend |
| `RIVET_NAMESPACE` | `namespace` | Namespace for actor isolation |
| `RIVET_TOKEN` | `token` | Authentication token for engine connection |
| `RIVET_PUBLIC_ENDPOINT` | `fetchHandler({ publicEndpoint })` | Client-facing endpoint |
| `RIVET_PUBLIC_TOKEN` | `fetchHandler({ publicToken })` | Client-facing token |
