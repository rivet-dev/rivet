...
description: "Multi-tenant data isolation with one Rivet Actor per tenant. The actor key is the tenant id, so each tenant gets its own actor and its own isolated dataset with no shared tables. Covers JSON state vs SQLite vs Drizzle, migrations, and deriving the tenant id from auth."
