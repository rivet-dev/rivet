---
title: "AI Agent"
description: "Patterns for building an AI agent backend with persistent conversation memory: one Rivet Actor per conversation, queue-driven message handling, streaming LLM responses as realtime events, and sandboxed coding agents."
templates: ["ai-agent"]
---
...
<AccordionGroup>
<Accordion title='agentManager["primary"]'>
...
- **Add authentication**: The examples have no auth, so anyone who reaches the server can create agents, list them, and message any agent whose key they can guess. Add `onAuth` and scoped tokens as a recommended extension. See [Authentication](/docs/actors/authentication).
