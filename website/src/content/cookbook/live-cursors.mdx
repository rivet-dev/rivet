...
description: "Patterns for live cursors and multiplayer presence with Rivet Actors: per-connection cursor state, realtime cursor sharing over typed events or raw WebSockets, presence join and leave cleanup, and update throttling guidance."
templates: ["cursors", "cursors-raw-websocket"]
---
...
<AccordionGroup>
<Accordion title="cursorRoom[roomId] (cursors)">
...
<Accordion title="cursorRoom[roomId] (cursors-raw-websocket)">
...
- **Actions**
  - `getOrCreate` (stub returning `{ status: "ok" }`, used only so the frontend can resolve the actor ID before dialing the WebSocket)
  - `getRoomState`
- **Queues**
  - None
- **State**
...
<Tabs>
<Tab title="cursors (Actions + Events)">
...
<Tab title="cursors-raw-websocket">
