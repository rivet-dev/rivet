---
title: "Chat Room"
description: "Build a realtime chat room backend with Rivet Actors. One actor per room key, SQLite-backed message history, and WebSocket broadcast of new messages to every connected client, plus extension patterns for typing indicators, presence, and per-user inboxes."
templates: ["chat-room"]
---
...
- The actor declares `events: { newMessage: event<Message>() }`, where `Message` is `{ sender, text, timestamp }`.
...
<AccordionGroup>
<Accordion title="chatRoom[roomId]">
...
sequenceDiagram
	...
	Note over R: first start runs onMigrate (CREATE TABLE messages)
	A->>R: getHistory()
