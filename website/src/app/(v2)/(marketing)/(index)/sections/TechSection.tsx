...
function TechLink({ href, name, icon, alt, external, status }: TechLinkProps) {
  ...
  return (
    <Link href={href} className={baseClasses} {...linkProps}>
      {status && (
        <span
          className={`absolute -right-1.5 -top-1.5 rounded px-1.5 py-0.5 text-[10px] ${statusClass} font-medium`}
        >
          {statusText}
        </span>
      )}
      <Image src={icon} alt={alt} width={22} height={22} className='object-contain' />
...
function TechSubSection({ title, children }: TechSubSectionProps) {
  return (
    <div className='mx-auto max-w-full lg:ml-auto lg:max-w-md'>
      {title && <h3 className='font-600 mb-3 text-lg text-white/80'>{title}</h3>}
      <div className='grid grid-cols-1 grid-cols-2 gap-2.5 md:grid-cols-4 lg:grid-cols-2'>{children}</div>
    </div>
  );
}
...
function TechSectionGroup({ children }: TechSectionGroupProps) {
  return <div className='grid grid-cols-1 items-start gap-12 lg:grid-cols-2'>{children}</div>;
}
...
function TechSectionText({ heading, description, linkText, linkHref, linkExternal }: TechSectionTextProps) {
  ...
  return (
    <div className='space-y-6'>
      <h2 className='font-700 text-2xl text-white sm:text-3xl'>{heading}</h2>
      <div className='space-y-4'>
        <p className='font-500 text-lg leading-relaxed text-white/40'>{description}</p>
        <p className='font-500 text-lg leading-relaxed text-white/40'>
        ...
        <TechSectionGroup>
          ...
          <TechSectionSubsections>
            ...
            <TechSubSection title='Frameworks'>
              ...
              <TechLink href='/docs/integrations/elysia' name='Elysia' icon={elysiaLogo} alt='Elysia' />
              <TechLink href='/docs/integrations/trpc' name='tRPC' icon={trpcLogo} alt='tRPC' />
            </TechSubSection>

            <TechSubSection title='Runtimes'>
              <TechLink
                href='/docs/actors/quickstart/backend'
                name='Node.js'
                icon={nodejsLogo}
            ...
            <TechSubSection title='Tools'>
              ...
              />
              <TechLink
                href='https://rivet-dev.github.io/rivetkit/'
                name='TypeDoc'
                icon={typescriptLogo}
                alt='TypeDoc'
                external
