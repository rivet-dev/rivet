...
export function NewUseCases() {
	...
	return (
		<section className="py-24 md:py-32 overflow-hidden">
			...
			<div className="mt-16 relative -mx-4 px-4 [mask-image:_linear-gradient(to_right,transparent_0,_black_10%,_black_90%,transparent_100%)]">
				<div
					ref={scrollRef}
					className="overflow-x-auto scrollbar-hide cursor-grab active:cursor-grabbing py-2"
				...
				>
					<div className="flex gap-6">
						...
						{useCases.map((useCase, index) => (
							<div key={`original-${index}`} className="flex-none w-72 md:w-80 bento-box border border-border rounded-xl p-6 select-none">
						...
						{useCases.map((useCase, index) => (
							<div key={`duplicate-${index}`} className="flex-none w-72 md:w-80 bento-box border border-border rounded-xl p-6 select-none">
								<h3 className="font-heading text-xl font-bold text-text-primary">{useCase.title}</h3>
								<p className="mt-2 text-text-secondary">{useCase.description}</p>
							</div>
						))}
