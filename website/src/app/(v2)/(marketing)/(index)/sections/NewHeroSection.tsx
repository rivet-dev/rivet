...
// UserNode Component
const UserNode = ({ label, position, style }: { label: string; position: string; style?: React.CSSProperties }) => (
	<div className={`absolute ${position} flex items-center space-x-3`} style={{ zIndex: 3, ...style }}>
		<div className="w-10 h-10 rounded-full bg-neutral-700 border-2 border-neutral-500 flex items-center justify-center">
...
const RequestPacket = ({
	...
	return (
		<div
			className={`absolute w-5 h-5 rounded-full bg-blue-500 border-2 border-blue-300 shadow-lg shadow-blue-500/50 ${position || ""} ${status === "flying" ? animationClass : "opacity-0"}`}
			style={{ 
...
const GreenPacket = ({ status }: { status: string }) => {
	return (
		<div
			className={`absolute w-5 h-5 rounded-full bg-green-500 border-2 border-green-300 shadow-lg shadow-green-500/50 ${status === "processing" ? "animate-fly-green" : "opacity-0"}`}
			style={{ 
				zIndex: 1,
		...
		<div className="relative w-full h-[400px] lg:h-[500px] flex items-center justify-center">
			<div className="relative w-[600px] h-[350px]">
				...
				<svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
					...
					<path
						d="M 376 221 L 376 310"
						className="connection-line-green"
						fill="none"
					/>
				</svg>
				...
				<div className="absolute left-[280px] top-[125px]" style={{ transform: "translateY(-50%)", zIndex: 2 }}>
					<div className={`relative ${getActorClasses()}`} style={{ borderRadius: "42px" }}>
						...
						<div className="mt-4 p-4 bg-black/30 rounded-lg w-[85%]">
							<code className="text-sm">
								<span className={getActorTextClasses()}>{"{"}</span>
								<span className="text-white/80">"count"</span>
								<span className="text-white">: </span>
								<span className="text-fuchsia-400 font-bold">{JSON.stringify(actorState.count)}</span>
								<span className={getActorTextClasses()}>{"}"}</span>
							</code>
						</div>
						<span className={`text-xs mt-2 font-medium ${getActorTextClasses()}`}>Status: {actorStatus}</span>
					</div>
				</div>
				...
				<div className="absolute left-[376px] bottom-0" style={{ transform: "translateX(-50%)", zIndex: 3 }}>
					<div className="flex items-center space-x-3">
						...
						<span className="text-green-400 text-sm font-medium">Database/API</span>
					</div>
				</div>
						...
						<div className="flex flex-col space-y-8 text-left">
							<h1 className="font-heading text-5xl md:text-6xl lg:text-7xl font-bold tracking-tighter" style={{ color: '#FAFAFA' }}>
								<span className="block overflow-hidden leading-[1.1]">
									<span className="block opacity-0 animate-hero-line" style={{ animationDelay: "0.1s" }}>
										Stateful Backends.
									</span>
								</span>
								<span className="block overflow-hidden leading-[1.1]">
									<span className="block opacity-0 animate-hero-line" style={{ animationDelay: "0.3s" }}>
										Finally Solved.
									</span>
								</span>
							</h1>
							...
							<div className="flex flex-col sm:flex-row gap-4 opacity-0 animate-hero-cta">
								<Link
									...
									className="px-8 py-3.5 rounded-lg font-bold text-white transition-all duration-200 hover:bg-orange-600 hover:shadow-lg hover:-translate-y-0.5 text-center"
								...
								<Link
									...
									className="px-8 py-3.5 rounded-lg font-medium transition-all duration-200 text-center"
									style={{
