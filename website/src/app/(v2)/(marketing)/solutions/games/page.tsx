...
const Badge = ({ text, color = "red" }) => {
	...
	return (
		...
		>
			<span className={`w-1.5 h-1.5 rounded-full ${color === "orange" ? "bg-orange-400" : color === "blue" ? "bg-blue-400" : color === "purple" ? "bg-purple-400" : color === "emerald" ? "bg-emerald-400" : "bg-red-400"} animate-pulse`} />
			{text}
		</div>
	);
};
								...
								<span className="table-cell">
									{(() => {
										...
										parts.forEach((part, j) => {
											...
											else if (["actor", "broadcast", "spawn", "rpc", "connectionId"].includes(trimmed)) {
												tokens.push(<span key={j} className="text-blue-400">{part}</span>);
											}
											// Object Keys / Properties / Methods
											else if (["state", "actions", "players", "scores", "map", "join", "move", "name", "hp", "pos", "x", "y", "isValidMove"].includes(trimmed)) {
...
// --- Refined Game Card matching landing page style with color highlights ---
					...
					<div className="relative h-80 rounded-2xl border border-white/10 bg-zinc-900/20 flex items-center justify-center overflow-hidden p-8">
						...
						<div className="relative z-10 flex flex-col items-center">
							...
							<div className="absolute w-full h-full flex items-center justify-center pointer-events-none">
								...
								<div className={`absolute w-48 h-48 border border-red-500/50 rounded-full transition-all duration-500 opacity-0 ${tick % 2 === 0 ? "scale-100 opacity-100" : "scale-50"}`} />
							</div>
						</div>
						...
						<div className="absolute inset-0">
							...
							<div className="absolute top-1/2 left-8 -translate-y-1/2 flex flex-col items-center gap-2">
								...
								{/* Incoming Input P1 (Odd ticks: visible at start, moves right) */}
								<div className={`absolute left-full top-1/2 w-2 h-2 bg-blue-400 rounded-full transition-all duration-300 ${tick % 2 !== 0 ? "translate-x-16 opacity-0" : "translate-x-0 opacity-100"}`} />
							</div>
							...
							<div className="absolute top-1/2 right-8 -translate-y-1/2 flex flex-col items-center gap-2">
								...
								{/* Incoming Input P2 (Even ticks: visible at start, moves left) */}
								<div className={`absolute right-full top-1/2 w-2 h-2 bg-green-400 rounded-full transition-all duration-300 ${tick % 2 === 0 ? "-translate-x-16 opacity-0" : "translate-x-0 opacity-100"}`} />
							</div>
						</div>
						...
						<div className="absolute bottom-4 left-4 right-4 h-16 bg-black/80 border border-white/10 rounded font-mono text-[10px] p-2 text-zinc-400 overflow-hidden backdrop-blur-md">
							...
							<div className="text-red-500">server.tick({tick}) &gt; Broadcasting State Snapshot</div>
							<div className="opacity-80">
								{tick % 2 !== 0 ? (
									<span className="text-blue-400">&lt; P1 Input: Move(x: 12, y: 40)</span>
								) : (
									<span className="text-green-400">&lt; P2 Input: Attack(target: P1)</span>
								)}
							</div>
						</div>
					</div>
				...
				>
					...
					<div className="relative rounded-2xl border border-white/10 bg-zinc-900 p-6 shadow-2xl">
						<div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
							...
							<div className="px-2 py-1 rounded bg-red-500/10 text-red-400 text-xs border border-red-500/20">Live</div>
						</div>
						<div className="space-y-4 text-sm font-mono">
							...
							<div className="w-full bg-zinc-800 rounded-full h-1.5 overflow-hidden">
								<div className="bg-red-500 h-1.5 rounded-full animate-[pulse_2s_infinite]" style={{ width: "92%" }} />
							</div>
	...
	<section className="py-24 bg-zinc-900/20 border-t border-white/5 relative overflow-hidden">
		<div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
			...
			<div className="flex flex-wrap justify-center gap-4">
				{["Unity", "Unreal Engine", "Godot", "Phaser", "Three.js", "PlayCanvas"].map((tech, i) => (
					<motion.div
						key={tech}
						initial={{ opacity: 0, y: 20 }}
						whileInView={{ opacity: 1, y: 0 }}
