...
const SPEC_PATH = resolve(
	import.meta.dirname,
	"../../sdks/api/full/openapi/openapi.yml",
);
...
const ERRORS_DIR_PATH = resolve(
	import.meta.dirname,
	"../../packages/common/formatted-error/errors",
);
...
async function generateApiDocs(
	...
	for (const pathName in SPEC.paths) {
		for (const method in SPEC.paths[pathName]) {
			const specPath = SPEC.paths[pathName][method];

			if (!/^(actors|builds|regions|routes)_/.test(specPath.operationId))
				continue;

			console.log("Registering", method, pathName);

			const fullUrl = apiBaseUrl + pathName;

			// Remove product prefix from operation ID
			const operationIdStripped = specPath.operationId
				.replace(`ee_`, "")
				.split("_");
      ...
      `;
			...
			const title = specPath.operationId.replace(/_/g, ".");
			file += `# ${title}\n\n`;
        ...
        `;
			...
			const requestBody =
				specPath.requestBody?.content["application/json"]?.schema;
			const responseBody =
      ...
      `;
			...
			if (specPath.parameters) {
				...
				for (const parameter of specPath.parameters) {
			...
			const docName = camelToKebab(operationIdStripped.join("/"));
			const filePath = join(OUTPUT_PATH, `${docName}.mdx`);

			await Deno.mkdir(dirname(filePath), { recursive: true });
			await Deno.writeTextFile(filePath, file);

			// Sort by grouping similar endpoints together
			const sortingKey = `${pathName} ${method}`;
	...
	await Deno.writeTextFile(join(OUTPUT_PATH, "index.mdx"), indexContent);
...
async function processErrorDir(inputPath, pages) {
	...
	for await (const dirEntry of Deno.readDir(inputPath)) {
		const inputPathEntry = join(inputPath, dirEntry.name);
		...
		if (dirEntry.isFile && dirEntry.name.endsWith(".md")) {
			...
			// Read metadata
			const titleMatch = errorDoc.match(/^#\s+(.*)$/m);
			const title = titleMatch ? titleMatch[1] : null;
			if (!title) throw new Error(`Missing title: ${inputPathEntry}`);

			const nameMatch = errorDoc.match(/^name\s*=\s*"([\w_]+)"\s*$/m);
			const name = nameMatch ? nameMatch[1] : null;
			if (!name) throw new Error(`Missing name: ${inputPathEntry}`);

			const httpStatusMatch = errorDoc.match(
				/^http_status\s*=\s*(\d+)\s*$/m,
			);
			...
			const isDeprecated = errorDoc.match(/^deprecated\s*=\s*true\s*$/m);
			const isExperimental = errorDoc.match(
				/^experimental\s*=\s*true\s*$/m,
			);
