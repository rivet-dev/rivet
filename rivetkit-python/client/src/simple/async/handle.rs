...
impl ActorHandle {
	...
	pub fn action<'a>(
		...
		pyo3_async_runtimes::tokio::future_into_py(py, async move {
			...
			let mut result =
				Python::with_gil(|py| match util::json_to_py_value(py, &vec![result]) {
					Ok(value) => Ok(value
						.iter()
	...
	pub fn subscribe<'a>(&self, py: Python<'a>, event_name: &str) -> PyResult<Bound<'a, PyAny>> {
		...
		pyo3_async_runtimes::tokio::future_into_py(py, async move {
			handle
				.on_event(&event_name.clone(), move |args| {
