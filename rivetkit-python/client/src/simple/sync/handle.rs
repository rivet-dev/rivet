...
impl ActorHandle {
	...
	pub fn action<'a>(
		...
		let result = self
			.handle
			.action(method, util::py_to_json_value(py, &args)?);
		let result = SYNC_RUNTIME.block_on(result);

		let Ok(result) = result else {
			return Err(py_runtime_err!("Failed to call action: {:?}", result.err()));
		};

		let mut result = util::json_to_py_value(py, &vec![result])?;
		let Some(result) = result.drain(0..1).next() else {
			return Err(py_runtime_err!("Expected one result, got {}", result.len()));
		};

		Ok(result)
	}

	pub fn subscribe(&self, event_name: &str) -> PyResult<()> {
		let event_name = event_name.to_string();
		let tx = self.event_tx.clone();

		SYNC_RUNTIME.block_on(self.handle.on_event(&event_name.clone(), move |args| {
			...
			tokio::spawn(async move {
				...
				// Send this upstream(?)
				tx.send(event)
					.map_err(|e| py_runtime_err!("Failed to send via inner tx: {}", e))
					.ok();
			});
		}));
	...
	pub fn receive<'a>(
		...
		let mut rx = self
			.event_rx
			.take()
			.ok_or_else(|| py_runtime_err!("Two .receive() calls cannot co-exist"))?;
	...
	pub fn disconnect(&self) {
		SYNC_RUNTIME.block_on(self.handle.disconnect())
	}
}
