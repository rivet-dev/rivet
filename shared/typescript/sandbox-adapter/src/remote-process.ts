 ...
 * timeout, and collects stdout and stderr. The agentOS actions and the
 * sandbox-agent HTTP API both keep output by sequence number but give an
 * actor no exit notification it can await, so this reads on an interval.
 */
...
async function killQuietly(process: RemoteProcess): Promise<void> {
	try {
		...
		// The process may have exited between the last poll and the kill.
	}
}
