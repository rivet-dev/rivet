...
pub async fn pegboard_actor_list_for_env(
	...
	let actors = ctx
		...
		.run(|tx, _mc| async move {
			...
			// Active actor idx
			if !input.include_destroyed {
				let actor_subspace =
					keys::subspace().subspace(&keys::env::ActiveActorKey::subspace(input.env_id));
				let (start, end) = actor_subspace.range();

				let end = if let Some(created_before) = input.created_before {
					keys::subspace().pack(&keys::env::ActiveActorKey::new(
						input.env_id,
						created_before,
						Uuid::nil(),
					))
				...
				while let Some(entry) = stream.try_next().await? {
					let actor_key = keys::subspace()
						.unpack::<keys::env::ActiveActorKey>(entry.key())
