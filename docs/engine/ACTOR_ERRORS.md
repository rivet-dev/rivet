...
The **runner pool error tracker** monitors serverless endpoint health. When errors occur, it stores them with timestamps. Errors are cleared after 3 consecutive successes (hysteresis). Errors older than 5 minutes are considered stale and ignored.
...
3. If pool has active (non-stale) error, return `PoolError` with details
...
3. If active (non-stale) error exists, return `actor_runner_failed` immediately
...
2. If active (non-stale) error exists, populate `pool_error` field
