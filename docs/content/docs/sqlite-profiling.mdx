...
```promql
sum by (actor_name, type, outcome) (
  rate(rivet_rivetkit_sqlite_operations_total{outcome!="success"}[5m])
)
```

These aggregate counts remain exact when the fingerprint series budget is exhausted. To break admitted operations down by fingerprint, use:
