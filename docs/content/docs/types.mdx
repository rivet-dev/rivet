...
| `run` | `RunContextOf` |
| `workflow` root context | `WorkflowContextOf` |
| `workflow` `loop` iterations | `WorkflowLoopContextOf` |
| `workflow` `try`, `race`, and `join` branches | `WorkflowBranchContextOf` |
...
Workflow context extractors are exported from [`@rivet-dev/workflows`](/workflows/docs).
