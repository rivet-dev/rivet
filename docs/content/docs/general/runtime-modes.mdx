...
On startup, your backend calls `registry.startEnvoy()` (or `registry.start()`, which selects Runner mode by default) to open a persistent connection to the Rivet control plane. When a client creates an actor, the control plane sends a command through this connection to start the actor on your backend.
...
<CodeSnippet file="examples/docs/general-runtime-modes/runner-pool.ts" />

See [Pool Configuration](/actors/docs/general/pool-configuration) for how pools are scaled, drained on version upgrades, and rate-limited during actor eviction.

## Serverless

Serverless runs actor logic in response to HTTP requests from Rivet, allowing your infrastructure to scale automatically. It is used on serverless platforms and is set automatically by Rivet Cloud, which sets `RIVETKIT_RUNTIME_MODE=serverless` on deploy.
...
<CodeSnippet file="examples/docs/general-runtime-modes/serverless-direct.ts" title="Direct" />

<CodeSnippet file="examples/docs/general-runtime-modes/serverless-router.ts" title="With-Router" />

</CodeGroup>

See [Server Setup](/actors/docs/general/http-server/) for more configuration options.

### Architecture

When a client creates an actor, it sends a request to the Rivet control plane. The control plane then calls `GET /api/rivet/start` on your serverless backend to run the actor.
...
When the request nears its lifespan, the control plane reserves a grace period (`serverless_drain_grace_period`, default: 10 seconds) at the end to gracefully stop actors. For example, with a 3600-second lifespan, actors begin stopping at 3590 seconds. After the full lifespan elapses, the connection is forcibly closed and any remaining actors are rescheduled.

See [Limits](/actors/docs/limits#serverless-shutdown) for configuration details.
