...
Integer parameters, including `bigint` values, must fit within JavaScript's safe integer range. Larger integers are rejected to prevent silent rounding.
...
Calls with parameters accept one SQL statement. Without parameters, a SQL script returns rows from its last result-producing statement.
...
Use `transactionSync(...)` when multiple synchronous queries must commit or roll back together. The transaction commits before the method returns and rolls back if the callback throws or returns a promise.

<CodeSnippet file="examples/docs/actors-sqlite/synchronous.ts" />

The callback must complete synchronously and use its `tx` value, which exposes only `executeSync(...)`. Do not start asynchronous work inside it. Rejecting a returned promise cannot cancel work that the callback already started. The transaction handle cannot be used after commit or rollback.

Return from the callback to commit or throw to roll back. Manual `BEGIN`, `COMMIT`, `END`, and `ROLLBACK` statements are rejected inside managed transactions; savepoints are supported.

If SQLite automatically rolls back a transaction, such as with `INSERT OR ROLLBACK`, catching the SQL error does not reopen it. Later queries through that transaction handle fail instead of committing outside the transaction.

`{ name, timeout }` options match `transaction(...)`. Nested synchronous transactions are unsupported. A synchronous call that would wait for another transaction fails immediately; use the asynchronous API to wait without blocking the worker. Synchronous operations are unavailable in WebAssembly runtimes.

Drizzle clients expose the same raw `executeSync(...)` and `transactionSync(...)` methods. Drizzle query builders remain asynchronous and cannot be used inside `transactionSync(...)`.
