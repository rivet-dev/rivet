...
See [Schedule & Cron](/actors/docs/schedule) for the full API, history, cancellation, failure behavior, and limits.

## Calendar job

Use `cron.set` instead of manually re-arming a one-shot action:

<CodeSnippet file="examples/docs/cookbook-cron-jobs/daily-report.ts" />
...
Keep scheduled actions idempotent when duplicate work would be harmful. See [Execution behavior](/actors/docs/schedule#execution-behavior) for retry behavior and workflow guidance. For durable multi-step jobs, use [workflows](/workflows/docs).
