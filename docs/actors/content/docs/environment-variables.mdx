...
| Environment Variable | Description |
|---------------------|-------------|
| `RIVETKIT_METRICS_ENABLED` | Set to `1` to enable the Prometheus `/metrics` route on the serverless handler. |
| `RIVETKIT_METRICS_TOKEN` | Bearer token that gates the serverless `/metrics` route. When unset, the route is disabled. |
