...
The layer above it decides which actor a client may reach at all. See [Authentication](/actors/docs/authentication) for that, and [JWTs](/actors/docs/jwt) to scope a client to a single actor before it ever gets here.
...
If you scope each user's [JWT](/actors/docs/jwt) to a single actor, that actor has exactly one participant and needs little authorization of its own. These hooks earn their keep once an actor is shared, as in a chat room or a per-tenant database.
