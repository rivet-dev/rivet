...
Authenticate the user however you already do, resolve the actor they own, then request a token from the control plane. Return only the JWT to the client.
...
Issuing requires the admin token, so it can only be done from your backend. `duration` defaults to 1 hour and is capped at 24. `subject` is your user identifier, opaque to Rivet, and shows up in token inspection.

## Grants

A grant answers one question: which operations may the holder perform on which resource. `target` is `"any"` for every instance in the namespace, or `{ id }` for one.
...
Operations are a subset of `create`, `read`, `update`, `delete`, and `list`. A grant set is capped at 32 grants, 5 operations each, and 2048 bytes encoded.
