/*
 * rivet-api-public
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 25.7.3
 * Contact: developer@rivet.gg
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct RunnerConfigsUpsertRequestBodyValueOneOfServerless {
    #[serde(rename = "headers", skip_serializing_if = "Option::is_none")]
    pub headers: Option<std::collections::HashMap<String, String>>,
    #[serde(rename = "max_runners")]
    pub max_runners: i32,
    #[serde(rename = "min_runners", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub min_runners: Option<Option<i32>>,
    /// Seconds.
    #[serde(rename = "request_lifespan")]
    pub request_lifespan: i32,
    #[serde(rename = "runners_margin", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub runners_margin: Option<Option<i32>>,
    #[serde(rename = "slots_per_runner")]
    pub slots_per_runner: i32,
    #[serde(rename = "url")]
    pub url: String,
}

impl RunnerConfigsUpsertRequestBodyValueOneOfServerless {
    pub fn new(max_runners: i32, request_lifespan: i32, slots_per_runner: i32, url: String) -> RunnerConfigsUpsertRequestBodyValueOneOfServerless {
        RunnerConfigsUpsertRequestBodyValueOneOfServerless {
            headers: None,
            max_runners,
            min_runners: None,
            request_lifespan,
            runners_margin: None,
            slots_per_runner,
            url,
        }
    }
}

