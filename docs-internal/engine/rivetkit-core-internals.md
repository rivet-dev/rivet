...
- `RegistryDispatcher::handle_fetch` owns framework HTTP routes `/metadata`, `/health`, `/inspector/*`, `/action/*`, and `/queue/*`. Prometheus metrics are process-wide and are never served by core itself; `metrics::render_prometheus_metrics()` only backs the NAPI `metrics()` binding and `Registry::prometheus_metrics()` so hosts can mount the rendered response on their own router. TypeScript NAPI callbacks keep action/queue schema validation and queue `canPublish`.
