...
- `RegistryDispatcher::handle_fetch` owns framework HTTP routes `/metadata`, `/health`, `/inspector/*`, `/action/*`, and `/queue/*`. Prometheus metrics are process-wide and are not served per actor; `metrics_endpoint::render_prometheus_metrics()` backs the NAPI `metrics()` binding and the serverless `/metrics` route instead. TypeScript NAPI callbacks keep action/queue schema validation and queue `canPublish`.
