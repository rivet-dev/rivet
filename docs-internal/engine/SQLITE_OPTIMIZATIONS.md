...
- RivetKit Prometheus metrics expose VFS read counters, fetched bytes, cache hits/misses, and `get_pages` duration. Render them from the hosting process via `registry.routes.prometheusMetrics()` and mount that on your own router; RivetKit does not serve a `/metrics` route itself.
