...
|---------|-------------|--------------------------------|
| Agent Class and Lifecycle | Base class, lifecycle hooks, and core properties for agent instances | `actor({...})` + lifecycle hooks |
| State Management | Persistent, synchronized, bidirectional state with validation support | Actor `state` + `connections` + `events` |
| SQL Database (Embedded SQLite) | Per-instance embedded SQLite with zero-latency local access | not possible atm |
| Callable Methods (RPC) | Decorator-based RPC methods invocable over WebSocket from clients | Actor `actions` |
| WebSocket Support | Bidirectional real-time connections with broadcasting and connection tags | `connect()` + events or low-level `onWebSocket` |
| Hibernation | Sleep when inactive, wake on messages while preserving WebSocket connections | Lifecycle sleep/wake hooks (no connection-preserving hibernation) |
| HTTP and Server-Sent Events (SSE) | HTTP request handling and server-to-client streaming via SSE | `onRequest` for HTTP; not possible atm for SSE |
| Routing | URL-based routing to agent instances with automatic name conversion | Actor keys + app/router mapping to keys |
| Scheduling | Delayed, cron, and interval task scheduling persisted to SQLite | `c.schedule.after()` / `c.schedule.at()` |
| Queue Tasks | Sequential FIFO task queue with retries stored in SQLite | Coordinator/data actor queue pattern + `c.schedule` retries |
| AI Chat Agent | Persistent chat with resumable streaming and tool support via AIChatAgent | not possible atm |
| Tool Use / Function Calling | Server-side and client-side tool execution with approval workflows | Actor `actions` for server-side tool execution patterns |
| AI Model Integration | Multi-provider AI model access including Workers AI and external APIs | not possible atm |
| Workflows | Durable multi-step background processing with retries and state updates | Actor orchestration via `c.client()` + `state` + `c.schedule` |
| Human-in-the-Loop | Workflow approval and MCP elicitation for pausing on human input | Stateful approval flows via `actions`/`onRequest` + `c.schedule` |
| Client SDK | React hooks, vanilla JS client, and HTTP fetch for agent connectivity | `rivetkit/client` + `@rivetkit/react` |
| Email Routing | Inbound email processing with routing, replying, and forwarding | not possible atm |
| Browse the Web (Headless Browser) | Web browsing via Puppeteer Browser Rendering API or Browserbase | not possible atm |
| Model Context Protocol (MCP) | Open standard for connecting AI systems to external tool servers | not possible atm |
| Protocol Messages | Automatic JSON frames sent on WebSocket connect for identity and state | not possible atm |
| Observability | Structured event emission for logging, monitoring, and remote services | `c.log` structured logging |
| Webhooks | Inbound webhook routing with signature verification and deduplication | `onRequest` webhook endpoints + custom signature validation |
| Configuration (Wrangler) | Wrangler-based config for bindings, migrations, secrets, and environments | `setup()`/registry/runtime configuration |
| x402 Payment Protocol | HTTP 402-based payment middleware for paywalled APIs and MCP tools | not possible atm |
| Agent Patterns | Prompt chaining, routing, parallelization, and orchestrator-worker patterns | Actor design patterns |
| Platform Limits | Account-level constraints on agents, storage, compute, and connections | Rivet Actor limits documentation |
| Deployment | CLI-based deploy with custom domains, preview deployments, and rollbacks | Connect deployment guides + `registry.serve()`/`handler()` |
| Getting Started (Quick Start) | Starter template setup with create-cloudflare CLI | Rivet Actors quickstart |
