...
use crate::error::{ScheduleRuntimeError, client_error_message};
...
impl ScheduleKind {
	...
	fn parse(value: &str, schedule_id: &str) -> Result<Self> {
		match value {
			"at" => Ok(Self::At),
			"cron" => Ok(Self::Cron),
			"every" => Ok(Self::Every),
...
impl ActorContext {
	...
	pub async fn at(&self, timestamp_ms: i64, action_name: &str, args: &[u8]) -> Result<String> {
		...
		self.sql()
			.execute(
				"INSERT INTO _rivet_schedule_events (event_id, trigger_at, action, args) VALUES (?, ?, ?, ?)",
				Some(vec![
	...
	pub async fn cancel_schedule(&self, event_id: &str) -> Result<bool> {
		...
		let result = self
			.sql()
			.execute(
				"DELETE FROM _rivet_schedule_events WHERE event_id = ? AND kind = 'at'",
	...
	pub async fn get_scheduled_event(&self, event_id: &str) -> Result<Option<ScheduledEventInfo>> {
		let result = self
			.sql()
			.query(
				"SELECT event_id, trigger_at, action, args, kind, cron_expression, timezone, interval_ms, last_started_at, max_history FROM _rivet_schedule_events WHERE event_id = ? AND kind = 'at'",
				Some(vec![BindParam::Text(event_id.to_owned())]),
			)
			.await
	...
	pub async fn list_scheduled_events(&self) -> Result<Vec<ScheduledEventInfo>> {
		let result = self
			.sql()
			.query(
				"SELECT event_id, trigger_at, action, args, kind, cron_expression, timezone, interval_ms, last_started_at, max_history FROM _rivet_schedule_events WHERE kind = 'at' ORDER BY trigger_at, event_id",
				None,
			)
			.await
	...
	async fn upsert_recurring(
		...
		self.sql()
			.execute(
				"INSERT INTO _rivet_schedule_events (event_id, trigger_at, action, args, kind, cron_expression, timezone, interval_ms, max_history) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(event_id) DO UPDATE SET trigger_at = excluded.trigger_at, action = excluded.action, args = excluded.args, kind = excluded.kind, cron_expression = excluded.cron_expression, timezone = excluded.timezone, interval_ms = excluded.interval_ms, max_history = excluded.max_history",
				Some(vec![
					BindParam::Text(event_id.to_owned()),
					BindParam::Integer(trigger_at),
					BindParam::Text(action_name.to_owned()),
					args_param(args),
					BindParam::Text(kind.as_str().to_owned()),
	...
	pub async fn cron_delete(&self, name: &str) -> Result<bool> {
		...
		let result = self
			...
			.execute(
				"DELETE FROM _rivet_schedule_events WHERE event_id = ? AND kind != 'at'",
				Some(vec![BindParam::Text(cron_event_id(name))]),
			)
			.await
	...
	pub async fn cron_history(&self, name: &str, limit: Option<i64>) -> Result<Vec<CronFire>> {
		...
		let result = self
			.sql()
			.query(
				"SELECT action, scheduled_at, fired_at, finished_at, result, error FROM _rivet_schedule_history WHERE schedule_id = ? ORDER BY fired_at DESC, id DESC LIMIT ?",
				Some(vec![
	...
	pub(crate) async fn take_due_schedule_dispatches(&self) -> Result<Vec<DueScheduleDispatch>> {
		...
		for row in &result.rows {
			...
			let history_result = is_running.then_some("skipped").unwrap_or("running");
			let history_index =
	...
	pub(crate) async fn finish_schedule_dispatch(
		...
		let (result, error_blob) = match error {
		...
		if let Err(error) = self
			...
			.execute(
				...
				Some(vec![
					...
					BindParam::Text(result.to_owned()),
					error_blob.map(BindParam::Blob).unwrap_or(BindParam::Null),
	...
	pub(crate) async fn recover_interrupted_schedule_history(&self) -> Result<()> {
		...
		self.sql()
			.execute(
				"UPDATE _rivet_schedule_history SET finished_at = ?, result = 'error', error = ? WHERE result = 'running'",
				Some(vec![
					BindParam::Integer(self.schedule_now_timestamp_ms()),
					BindParam::Blob(encode_schedule_error(&error)?),
				]),
			)
			.await
...
fn append_history_statements(
	...
	statements.push(SqliteBatchStatement {
		sql: "INSERT INTO _rivet_schedule_history (schedule_id, action, scheduled_at, fired_at, finished_at, result) VALUES (?, ?, ?, ?, ?, ?)".to_owned(),
		params: Some(vec![
			BindParam::Text(event.event_id.clone()),
...
fn read_stored_schedule(row: &[ColumnValue]) -> Result<StoredSchedule> {
	...
	let kind = ScheduleKind::parse(&read_text(row, 4, "kind")?, &event_id)?;
...
fn read_cron_fire(row: &[ColumnValue]) -> Result<CronFire> {
	Ok(CronFire {
		...
		result: read_text(row, 4, "result")?,
		error: read_optional_blob(row, 5, "error")?
			.map(|value| decode_schedule_error(&value))
			.transpose()?,
	})
}
...
fn encode_schedule_error(error: &ScheduleErrorInfo) -> Result<Vec<u8>> {
	let mut output = Vec::new();
	ciborium::into_writer(error, &mut output).context("encode schedule history error")?;
	Ok(output)
}

fn decode_schedule_error(value: &[u8]) -> Result<ScheduleErrorInfo> {
	ciborium::from_reader(value).context("decode schedule history error")
}
