...
use crate::actor::callbacks::{
	ActorInstanceCallbacks, OnDestroyRequest, OnMigrateRequest, OnSleepRequest,
	OnStateChangeRequest, OnWakeRequest,
};
...
impl ActorLifecycle {
	pub async fn startup(
		...
		if let Some(on_before_actor_start) = options.driver_hooks.on_before_actor_start.as_ref() {
	...
	pub async fn shutdown_for_sleep(
		...
		ctx.wait_for_run_handler(config.effective_run_stop_timeout())
			.await;
		...
		if let Some(on_sleep) = callbacks.on_sleep.as_ref() {
			...
			match timeout(
				on_sleep_timeout,
				on_sleep(OnSleepRequest { ctx: ctx.clone() }),
			)
			.await
			{
				Ok(Ok(())) => {}
	...
	fn spawn_run_handler(&self, ctx: ActorContext, callbacks: Arc<ActorInstanceCallbacks>) {
	...
	async fn process_overdue_scheduled_events(
		...
		let invoker = ActionInvoker::with_shared_callbacks(factory.config().clone(), callbacks);
		ctx.schedule().handle_alarm(ctx, &invoker).await;
	}
}
