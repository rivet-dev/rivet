...
// frontend/dist/inspector-tab/ into $OUT_DIR/inspector-tab/ so the
// inspector handler can embed both via include_dir!. Falls back to an
// empty directory when the frontend has not been built yet, so a missing
// bundle degrades to 404 at runtime instead of a compile error.
fn main() -> Result<()> {
