...
mod moved_tests {
	...
	use super::{
		ActorLifecycle, ActorLifecycleDriverHooks, BeforeActorStartRequest, ShutdownStatus,
		StartupError, StartupOptions, StartupStage,
	};
	use crate::actor::callbacks::{
		ActorInstanceCallbacks, OnDestroyRequest, OnMigrateRequest, OnSleepRequest, OnWakeRequest,
		RunRequest,
	};
	use crate::actor::connection::{
		HibernatableConnectionMetadata, PersistedConnection, encode_persisted_connection,
		make_connection_key,
	};
	...
	async fn sleep_shutdown_waits_for_idle_window_and_persists_state() {
		...
		let factory = ActorFactory::new(config.clone(), move |_request| {
