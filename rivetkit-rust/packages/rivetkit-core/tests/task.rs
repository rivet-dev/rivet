			...
			logs.iter().any(|log| {
				log.level == tracing::Level::DEBUG
					...
					&& matches!(
						...
						Some("actor lifecycle command received")
							| Some("actor dispatch command received")
							| Some("actor event enqueued")
							| Some("actor event drained")
					)
			}),
			"expected at least one actor-task flow log for actor-log-flow; logs={logs:?}"
		);
	}
