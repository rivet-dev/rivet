...
mod moved_tests {
	...
	async fn actor_history_is_globally_bounded() {
		...
		ctx.sql()
			.execute(
				"WITH RECURSIVE n(value) AS (SELECT 1 UNION ALL SELECT value + 1 FROM n WHERE value < 10001) INSERT INTO _rivet_schedule_history (schedule_id, action, scheduled_at, fired_at, finished_at, result) SELECT 'cron:seed', 'seed', value, value, value, 'ok' FROM n",
				None,
			)
			.await
			.unwrap();
