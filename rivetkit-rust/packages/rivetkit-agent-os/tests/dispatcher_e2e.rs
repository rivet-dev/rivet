...
async fn dispatch_one(vm: &AgentOs, name: &str, args_cbor: Vec<u8>) -> Result<Vec<u8>, String> {
	...
	let action = match event {
		RuntimeEvent::Action(action) => action,
		other => panic!("expected Action event, got {other:?}"),
	};
	let mut vars = rivetkit_agent_os::actor::Vars::default();
	rivetkit_agent_os::actions::dispatch(&start.ctx, vm, &mut vars, action).await;
...
async fn dispatcher_round_trips_write_then_read_file() {
	...
	let write_args = {
		let mut buf = Vec::new();
		let tuple = (path.to_owned(), serde_bytes::ByteBuf::from(payload.clone()));
	...
	let unit: Option<serde_json::Value> = ciborium::from_reader(Cursor::new(write_reply)).ok();
