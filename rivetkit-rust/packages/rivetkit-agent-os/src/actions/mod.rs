...
///
/// `ctx` provides the actor's SQLite database, used by the persistence-backed
/// arms (signed preview URLs and session metadata in `agent_os_*` tables).
pub async fn dispatch(
	ctx: &Ctx<AgentOsActor>,
	vm: &AgentOs,
	vars: &mut Vars,
	action: ActionCall<AgentOsActor>,
) {
	...
	match name.as_str() {
		...
		"deleteFile" => {
			...
			let decoded = action
				...
				.or_else(|_| action.decode_as::<(String,)>().map(|(path,)| (path, false)));
			match decoded {
				Ok((path, recursive)) => {
		...
		"createSession" => {
			...
			let decoded = action
				...
				.or_else(|_| {
					action.decode_as::<(String,)>().map(|(agent_type,)| {
						(agent_type, session::CreateSessionOptionsDto::default())
					})
				});
			match decoded {
				Ok((agent_type, options)) => {
					match session::create_session(ctx, vm, vars, &agent_type, options).await {
						Ok(id) => action.ok(&id),
						Err(error) => {
		...
		"sendPrompt" => {
			...
			match args {
				Ok((session_id, text)) => {
					match session::send_prompt(ctx, vm, vars, &session_id, &text).await {
						Ok(result) => action.ok(&result),
						Err(error) => {
							tracing::error!(?error, session_id, "send_prompt failed");
							action.err(error)
						}
					}
				}
		...
		"closeSession" => {
			let args: Result<(String,)> = action.decode_as();
			match args {
				Ok((session_id,)) => match session::close_session(ctx, vm, vars, &session_id).await
				{
					Ok(()) => action.ok(&()),
					Err(error) => action.err(error),
				},
		...
		"listPersistedSessions" => match session::list_persisted_sessions(ctx).await {
			Ok(sessions) => action.ok(&sessions),
			Err(error) => action.err(error),
		},
		"getSessionEvents" => {
			let args: Result<(String,)> = action.decode_as();
			match args {
				Ok((session_id,)) => match session::get_session_events(ctx, &session_id).await {
					Ok(events) => action.ok(&events),
					Err(error) => action.err(error),
				},
				Err(error) => action.err(error),
			}
		...
		"createSignedPreviewUrl" => {
			// TS calls `createSignedPreviewUrl(port, ttlSeconds)`.
			let args: Result<(u16, u64)> = action.decode_as();
			match args {
				Ok((port, ttl_seconds)) => match preview::create(ctx, port, ttl_seconds).await {
					Ok(dto) => action.ok(&dto),
					Err(error) => action.err(error),
				},
				Err(error) => action.err(error),
			}
		...
		"expireSignedPreviewUrl" => {
			let args: Result<(String,)> = action.decode_as();
			match args {
				Ok((token,)) => match preview::expire(ctx, &token).await {
					Ok(()) => action.ok(&()),
					Err(error) => action.err(error),
				},
				Err(error) => action.err(error),
