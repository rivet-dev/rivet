...
/// Encode/decode round-trip for a structured non-byte payload modeled
/// after `agent_os_client::VirtualStat`. Phase 2 gate: every field
...
fn encode_then_decode_round_trips_virtual_stat() {
	...
	assert_eq!(
		revived["ino"].as_u64().expect("ino u64"),
		9_876_543_210u64,
	);
