...
fn write_fixture(name: &str, intermediate: &serde_json::Value) {
	...
	let serialized = serde_json::to_string_pretty(intermediate).expect("serialize fixture as JSON");
...
/// Structured non-byte payload modeled after a virtual filesystem stat result.
