...
interface RivetkitRuntimeModule {
	setup: (input: {
		use: Record<string, DynamicActorDefinitionLike>;
		serveManager: false;
		noWelcome: true;
		test: { enabled: false };
	}) => {
	...
	createActorRouter: (
		...
		fetch: (request: Request, params: { actorId: string }) => Promise<Response>;
	};
	routeWebSocket: (...args: unknown[]) => Promise<unknown>;
	InlineWebSocketAdapter: new (handler: unknown) => {
	...
	CONN_STATE_MANAGER_SYMBOL: symbol;
}
...
type DynamicActorRouter = ReturnType<RivetkitRuntimeModule["createActorRouter"]>;
...
function readBootstrapConfig(): DynamicBootstrapConfig {
	...
	if (
		...
		(configValue.sourceFormat !== "typescript" &&
...
function loadRivetkitRuntime(): RivetkitRuntimeModule {
	try {
		const packageName = "rivetkit";
		return require(packageName) as RivetkitRuntimeModule;
	} catch (error) {
