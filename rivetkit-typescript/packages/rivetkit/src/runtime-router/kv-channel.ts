// KV Channel WebSocket handler for the local runtime server.
//
...
import {
	PROTOCOL_VERSION,
	type ToServer as ToRivet,
	type ToClient,
	type RequestData,
	type ResponseData,
	type ToServerRequest as ToRivetRequest,
	decodeToServer as decodeToRivet,
	encodeToClient,
} from "@rivetkit/engine-kv-channel-protocol";
import { KvStorageQuotaExceededError } from "./kv-limits";
...
export interface KvChannelManager {
	createHandler: (engineClient: EngineControlClient) => {
			...
			return {
				...
				onMessage: (event: any, _ws: WSContext) => {
					try {
						...
						handleToRivetMessage(state, conn, engineClient, msg);
					} catch (err: unknown) {
						logger().error({
...
async function processRequestData(
	...
	switch (data.tag) {
		...
		case "KvDeleteRangeRequest": {
			...
			return await handleKvOperation(engineClient, actorId, data);
		}
	}
}
...
async function handleKvOperation(
	...
	switch (data.tag) {
		case "KvGetRequest": {
			...
			const results = await engineClient.kvBatchGet(actorId, keys);
		...
		case "KvPutRequest": {
			...
			try {
				await engineClient.kvBatchPut(actorId, entries);
			} catch (err: unknown) {
		...
		case "KvDeleteRequest": {
			...
			await engineClient.kvBatchDelete(actorId, keys);
		...
		case "KvDeleteRangeRequest": {
			...
			await engineClient.kvDeleteRange(actorId, start, end);
	...
	switch (msg.tag) {
		case "ToServerRequest": {
			...
			const next = prev.then(() =>
				handleRequest(state, conn, engineClient, msg.val).catch(
					(err) => {
						logger().error({
