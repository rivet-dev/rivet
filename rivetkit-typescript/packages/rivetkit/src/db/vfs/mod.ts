...
class SqliteSystem extends VFS.Base {
	...
	xRead(fileId: number, pData: Uint8Array, iOffset: number): number {
		return this.handleAsync(async () => {
			...
			vfsLog("xRead", { file: file.path, offset: iOffset, len: requestedLength, chunks: chunkKeys.length, ms: (performance.now() - readStart).toFixed(2) });
	...
	xWrite(fileId: number, pData: Uint8Array, iOffset: number): number {
		return this.handleAsync(async () => {
			...
			vfsLog("xWrite", { file: file.path, offset: iOffset, len: writeLength, readChunks: chunkKeys.length, writeEntries: entriesToWrite.length, getBatchMs: getBatchMs.toFixed(2), putBatchMs: (performance.now() - putBatchStart).toFixed(2) });
