 ...
 * Uses dynamic import to keep @rivetkit/sqlite-wasm tree-shakeable.
 */
export class SqliteVfsPoolManager {
	...
	async acquire(actorId: string): Promise<ISqliteVfs> {
		if (!this.#poolPromise) {
			...
			// @rivetkit/sqlite-wasm module graph at compile time. Without this,
			...
			const specifier = ["@rivetkit", "sqlite-wasm"].join("/");
