...
export function readHttpQueueSendResponse(bc: bare.ByteCursor): HttpQueueSendResponse {
    return {
        receiptId: bare.readString(bc),
        deduplicated: bare.readBool(bc),
    }
}

export function writeHttpQueueSendResponse(bc: bare.ByteCursor, x: HttpQueueSendResponse): void {
    bare.writeString(bc, x.receiptId)
    bare.writeBool(bc, x.deduplicated)
}
...
export type HttpQueueStatusRequest = {
    readonly receiptId: string
}

export function readHttpQueueStatusRequest(bc: bare.ByteCursor): HttpQueueStatusRequest {
    return {
        receiptId: bare.readString(bc),
    }
}

export function writeHttpQueueStatusRequest(bc: bare.ByteCursor, x: HttpQueueStatusRequest): void {
    bare.writeString(bc, x.receiptId)
}

export function encodeHttpQueueStatusRequest(x: HttpQueueStatusRequest, config?: Partial<bare.Config>): Uint8Array {
    const fullConfig = config != null ? bare.Config(config) : DEFAULT_CONFIG
    const bc = new bare.ByteCursor(
        new Uint8Array(fullConfig.initialBufferLength),
        fullConfig,
    )
    writeHttpQueueStatusRequest(bc, x)
    return new Uint8Array(bc.view.buffer, bc.view.byteOffset, bc.offset)
}

export function decodeHttpQueueStatusRequest(bytes: Uint8Array): HttpQueueStatusRequest {
    const bc = new bare.ByteCursor(bytes, DEFAULT_CONFIG)
    const result = readHttpQueueStatusRequest(bc)
    if (bc.offset < bc.view.byteLength) {
        throw new bare.BareError(bc.offset, "remaining bytes")
    }
    return result
...
export type HttpQueueStatusResponse = {
    readonly state: string
    readonly attempts: u64 | null
    readonly createdAt: u64 | null
    readonly availableAt: u64 | null
    readonly startedAt: u64 | null
    readonly completedAt: u64 | null
    readonly failedAt: u64 | null
    readonly consumedAt: u64 | null
}

export function readHttpQueueStatusResponse(bc: bare.ByteCursor): HttpQueueStatusResponse {
    return {
        state: bare.readString(bc),
        attempts: read4(bc),
        createdAt: read4(bc),
        availableAt: read4(bc),
        startedAt: read4(bc),
        completedAt: read4(bc),
        failedAt: read4(bc),
        consumedAt: read4(bc),
    }
}

export function writeHttpQueueStatusResponse(bc: bare.ByteCursor, x: HttpQueueStatusResponse): void {
    bare.writeString(bc, x.state)
    write4(bc, x.attempts)
    write4(bc, x.createdAt)
    write4(bc, x.availableAt)
    write4(bc, x.startedAt)
    write4(bc, x.completedAt)
    write4(bc, x.failedAt)
    write4(bc, x.consumedAt)
}

export function encodeHttpQueueStatusResponse(x: HttpQueueStatusResponse, config?: Partial<bare.Config>): Uint8Array {
    const fullConfig = config != null ? bare.Config(config) : DEFAULT_CONFIG
    const bc = new bare.ByteCursor(
        new Uint8Array(fullConfig.initialBufferLength),
        fullConfig,
    )
    writeHttpQueueStatusResponse(bc, x)
    return new Uint8Array(bc.view.buffer, bc.view.byteOffset, bc.offset)
}

export function decodeHttpQueueStatusResponse(bytes: Uint8Array): HttpQueueStatusResponse {
    const bc = new bare.ByteCursor(bytes, DEFAULT_CONFIG)
    const result = readHttpQueueStatusResponse(bc)
    if (bc.offset < bc.view.byteLength) {
        throw new bare.BareError(bc.offset, "remaining bytes")
    }
    return result
