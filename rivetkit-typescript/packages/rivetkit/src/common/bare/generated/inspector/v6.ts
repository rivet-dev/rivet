...
function read2(bc: bare.ByteCursor): ScheduleError | null {
    return bare.readBool(bc) ? readScheduleError(bc) : null
}

function write2(bc: bare.ByteCursor, x: ScheduleError | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
...
function read3(bc: bare.ByteCursor): readonly Connection[] {
...
function write3(bc: bare.ByteCursor, x: readonly Connection[]): void {
...
function read4(bc: bare.ByteCursor): State | null {
    return bare.readBool(bc) ? readState(bc) : null
}

function write4(bc: bare.ByteCursor, x: State | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        writeState(bc, x)
    }
...
function read6(bc: bare.ByteCursor): WorkflowHistory | null {
    return bare.readBool(bc) ? readWorkflowHistory(bc) : null
}

function write6(bc: bare.ByteCursor, x: WorkflowHistory | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
...
function read7(bc: bare.ByteCursor): readonly TabConfigEntry[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write7(bc: bare.ByteCursor, x: readonly TabConfigEntry[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
...
function read8(bc: bare.ByteCursor): readonly Schedule[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write8(bc: bare.ByteCursor, x: readonly Schedule[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
...
function read9(bc: bare.ByteCursor): readonly QueueMessageSummary[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write9(bc: bare.ByteCursor, x: readonly QueueMessageSummary[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
...
function read10(bc: bare.ByteCursor): readonly ScheduleFire[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write10(bc: bare.ByteCursor, x: readonly ScheduleFire[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
