...
import { createSqliteVfs } from "../fs/sqlite-vfs";
import type {
...
async function ensureVm<TConnParams>(
	...
	// Build options with SQLite-backed filesystem as default working directory
	// mount so VM filesystem state persists across sleep/wake cycles.
	const options = buildVmOptions(c.db, config.options);
...
function buildVmOptions(
	...
	// their override and skip the default SQLite VFS mount.
	...
	const sqliteVfs = createSqliteVfs({ db: actorDb });
	const sqliteMount: MountConfig = {
		type: "custom",
		path: "/home/user",
		backend: sqliteVfs,
	};
