		...
		return await new Promise<Array<Record<string, unknown>>>(
			...
			const timeout = setTimeout(() => {
				...
				const cleanup = () => {
					...
					ws.removeEventListener("message", onMessage as (event: any) => void);
					ws.removeEventListener("close", onClose as (event: any) => void);
				};
...
export function runHibernatableWebSocketProtocolTests(
	...
	describe.skipIf(
		...
		test(
			"replays only unacked indexed websocket messages after sleep and wake",
			async (c) => {
			...
			try {
				...
				await vi.waitFor(
					...
					{ timeout: HIBERNATABLE_ACK_SETTLE_TIMEOUT_MS, interval: 50 },
				);
				...
				expect((await actorObservedOrderPromise)[0].order).toEqual([1, 3, 4]);
			} finally {
				ws.close();
			}
			},
		...
		test(
			"cleans up stale hibernatable websocket connections on restore",
			async (c) => {
			if (driverTestConfig.clientType !== "http") {
				return;
			}
