...
`,
			...
			// --- createOptions factory path ---

			test("createOptions: writeFile and readFile round-trip", async (c) => {
				const { client } = await setupDriverTest(c, {
					...driverTestConfig,
					useRealTimers: true,
				});
				const actor = client.agentOsCreateOptionsTestActor.getOrCreate([
					`create-opts-fs-${crypto.randomUUID()}`,
				]);

				await actor.writeFile(
					"/home/user/hello.txt",
					"hello from createOptions",
				);
				const data = await actor.readFile("/home/user/hello.txt");
			...
			test("createOptions: exec runs a command", async (c) => {
				const { client } = await setupDriverTest(c, {
					...driverTestConfig,
					useRealTimers: true,
				});
				const actor = client.agentOsCreateOptionsTestActor.getOrCreate([
					`create-opts-exec-${crypto.randomUUID()}`,
				]);

				const result = await actor.exec("echo factory-path");
				expect(result.exitCode).toBe(0);
				expect(result.stdout.trim()).toBe("factory-path");
			}, 60_000);

			test("createOptions: two instances get independent VMs", async (c) => {
				const { client } = await setupDriverTest(c, {
					...driverTestConfig,
					useRealTimers: true,
				});

				const actorA = client.agentOsCreateOptionsTestActor.getOrCreate(
					[`create-opts-iso-a-${crypto.randomUUID()}`],
				);
				const actorB = client.agentOsCreateOptionsTestActor.getOrCreate(
					[`create-opts-iso-b-${crypto.randomUUID()}`],
				);

				// Write a file only in actor A.
				await actorA.writeFile("/home/user/only-in-a.txt", "a-data");

				// Actor B should not see it because each instance has its own VM.
				expect(await actorB.exists("/home/user/only-in-a.txt")).toBe(
					false,
				);

				// Actor A should still see it.
				expect(await actorA.exists("/home/user/only-in-a.txt")).toBe(
					true,
				);
			}, 60_000);
		},
