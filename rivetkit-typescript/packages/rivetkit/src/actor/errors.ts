...
export class RivetError extends Error {
	...
	constructor(
		...
		this.rayId = normalized.rayId ?? undefined;
...
/** A `RivetError` instance, or a plain object carrying its full tagged shape. */
export function isCanonicalStructuredRivetError(
	error: unknown,
): error is RivetErrorLike {
	return (
...
export function decodeBridgeRivetErrorPayload(
	...
	try {
		...
		) as NativeBridgeErrorPayload;
		// Coerce the native bridge's null ray ID to undefined before validation.
		const payload: BridgeRivetErrorPayload = {
			...raw,
			rayId: raw.rayId ?? undefined,
		};
