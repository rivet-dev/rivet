...
describe("assertCborSerializable", () => {
	...
	test("rejects a function", () => {
		expect(() => assertCborSerializable(() => {})).toThrow(TypeError);
	});

	test("rejects a nested function", () => {
		expect(() =>
			assertCborSerializable({ foo: () => {} }),
		).toThrow(TypeError);
	});

	test("rejects a symbol", () => {
		expect(() => assertCborSerializable(Symbol("x"))).toThrow(TypeError);
	});

	test("rejects a RegExp", () => {
		expect(() => assertCborSerializable(/abc/)).toThrow(TypeError);
	});

	test("rejects a WeakMap", () => {
		expect(() => assertCborSerializable(new WeakMap())).toThrow(TypeError);
	});

	test("rejects a WeakSet", () => {
		expect(() => assertCborSerializable(new WeakSet())).toThrow(TypeError);
	});

	test("rejects a Promise", () => {
		expect(() =>
			assertCborSerializable(Promise.resolve()),
		).toThrow(TypeError);
	});

	test("rejects a function inside a Map value", () => {
		expect(() =>
			assertCborSerializable(new Map([["k", () => {}]])),
		).toThrow(TypeError);
	});

	test("rejects a function inside a Set", () => {
		expect(() =>
			assertCborSerializable(new Set([() => {}])),
		).toThrow(TypeError);
	});

	test("rejects a function inside an array", () => {
		expect(() => assertCborSerializable([1, () => {}])).toThrow(TypeError);
	});
});
