...
describe("parsePortEnv", () => {
	...
	test("port 0 is rejected", () => {
		expect(() => parsePortEnv("0")).toThrow(/PORT env var must be/);
	});

	test("port 65536 is rejected", () => {
		expect(() => parsePortEnv("65536")).toThrow(/PORT env var must be/);
	});

	test("non-numeric input is rejected", () => {
		expect(() => parsePortEnv("notaport")).toThrow(/PORT env var must be/);
	});

	test("partial numeric input is rejected (parseInt would silently succeed)", () => {
		expect(() => parsePortEnv("8080abc")).toThrow(/PORT env var must be/);
	});

	test("negative input is rejected", () => {
		expect(() => parsePortEnv("-1")).toThrow(/PORT env var must be/);
	});
});
