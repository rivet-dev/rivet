...
/**
 * A TCP proxy in front of the engine. Taking it down drops a client's sockets
 * and refuses new ones, the way an engine restart or a network failure does,
 * while the actor keeps running on the other side.
 */
export interface TcpProxy {
	...
	down(): void;
	/** Resolves once `count` connection attempts were refused since `down()`. */
	refusedAttempts(count: number): Promise<void>;
	...
	const server = createServer((client) => {
		if (refusing) {
			...
			waiters = waiters.filter((waiter) => {
				if (refused < waiter.count) return true;
				waiter.resolve();
				return false;
			});
			return;
