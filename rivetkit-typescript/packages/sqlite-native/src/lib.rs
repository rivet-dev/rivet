...
use channel::{KvChannel, KvChannelConfig, OpMetrics};

// MARK: SQL Metrics

/// Per-SQL-statement-type timing for diagnosing napi + spawn_blocking overhead.
pub struct SqlMetrics {
	pub execute: OpMetrics,
	pub query: OpMetrics,
	pub exec: OpMetrics,
	pub spawn_blocking_wait: OpMetrics,
	pub sqlite_step: OpMetrics,
	pub stmt_cache: OpMetrics,
	pub result_serialize: OpMetrics,
}

impl SqlMetrics {
	pub fn new() -> Self {
		Self {
			execute: OpMetrics::new(),
			query: OpMetrics::new(),
			exec: OpMetrics::new(),
			spawn_blocking_wait: OpMetrics::new(),
			sqlite_step: OpMetrics::new(),
			stmt_cache: OpMetrics::new(),
			result_serialize: OpMetrics::new(),
		}
	}
}
...
pub fn connect(config: ConnectConfig) -> JsKvChannel {
	...
	JsKvChannel {
		...
		sql_metrics: Arc::new(SqlMetrics::new()),
	}
}
...
pub async fn open_database(
	...
	Ok(JsNativeDatabase {
		...
		sql_metrics: channel.sql_metrics.clone(),
		actor_id,
	})
}
...
pub async fn execute(
	...
	let sql_metrics = db.sql_metrics.clone();
	let trace_sql = std::env::var("RIVET_TRACE_SQL").is_ok();

	let result = db.rt_handle
		.spawn_blocking(move || {
			let blocking_wait = outer_start.elapsed();
			sql_metrics.spawn_blocking_wait.record(blocking_wait);
			if trace_sql {
				eprintln!("[sql-trace] execute spawn_wait={}us", blocking_wait.as_micros());
			}
			...
			sql_metrics.sqlite_step.record(step_elapsed);
			if trace_sql {
				eprintln!("[sql-trace] execute sqlite_step={}us", step_elapsed.as_micros());
			}
			...
			sql_metrics.stmt_cache.record(cache_start.elapsed());

			Ok(ExecuteResult { changes })
		})
		.await
		.map_err(|e| Error::from_reason(e.to_string()))??;
	db.sql_metrics.execute.record(outer_start.elapsed());
	Ok(result)
}
...
pub async fn query(
	...
	let result = db.rt_handle
		.spawn_blocking(move || {
			sql_metrics.spawn_blocking_wait.record(outer_start.elapsed());
			let guard = db_arc.lock().unwrap();
			let native_db = guard
				.as_ref()
				.ok_or_else(|| Error::from_reason("database is closed"))?;
			...
			sql_metrics.sqlite_step.record(step_start.elapsed());
		...
		.await
		.map_err(|e| Error::from_reason(e.to_string()))??;
	db.sql_metrics.query.record(outer_start.elapsed());
	Ok(result)
}
...
pub async fn exec(db: &JsNativeDatabase, sql: String) -> Result<QueryResult> {
	...
	let result = db.rt_handle
		...
		.await
		.map_err(|e| Error::from_reason(e.to_string()))??;
	db.sql_metrics.exec.record(outer_start.elapsed());
	Ok(result)
}
...
#[napi(object)]
pub struct KvChannelMetricsSnapshot {
	pub get: OpMetricsSnapshot,
	pub put: OpMetricsSnapshot,
	pub delete: OpMetricsSnapshot,
	pub delete_range: OpMetricsSnapshot,
	pub actor_open: OpMetricsSnapshot,
	pub actor_close: OpMetricsSnapshot,
	pub keys_total: i64,
	pub requests_total: i64,
	pub batch_atomic_commits: i64,
	pub batch_atomic_pages: i64,
}

/// SQL execution metrics (Layer 0).
#[napi(object)]
pub struct SqlMetricsSnapshot {
	pub execute: OpMetricsSnapshot,
	pub query: OpMetricsSnapshot,
	pub exec: OpMetricsSnapshot,
	pub spawn_blocking_wait: OpMetricsSnapshot,
	pub sqlite_step: OpMetricsSnapshot,
	pub stmt_cache: OpMetricsSnapshot,
	pub result_serialize: OpMetricsSnapshot,
}
...
pub struct VfsMetricsSnapshot {
	pub xread_count: i64,
	pub xread_us: i64,
	pub xwrite_count: i64,
	pub xwrite_us: i64,
	pub xwrite_buffered_count: i64,
	pub xsync_count: i64,
	pub xsync_us: i64,
	pub commit_atomic_count: i64,
	pub commit_atomic_us: i64,
	pub commit_atomic_pages: i64,
}

/// All metrics across all layers.
#[napi(object)]
pub struct AllMetricsSnapshot {
	pub kv_channel: KvChannelMetricsSnapshot,
	pub sql: SqlMetricsSnapshot,
	pub vfs: VfsMetricsSnapshot,
}

fn snapshot_op(op: &channel::OpMetrics) -> OpMetricsSnapshot {
	let (count, total, min, max) = op.snapshot();
	OpMetricsSnapshot {
		count: count as i64,
		total_duration_us: total as i64,
		min_duration_us: min as i64,
		max_duration_us: max as i64,
		avg_duration_us: if count > 0 { total as f64 / count as f64 } else { 0.0 },
	}
}

/// Get a snapshot of all metrics across all layers.
#[napi(js_name = "getMetrics")]
pub fn get_metrics(channel: &JsKvChannel) -> AllMetricsSnapshot {
	let m = channel.channel.metrics();
	let s = &*channel.sql_metrics;

	AllMetricsSnapshot {
		kv_channel: KvChannelMetricsSnapshot {
			get: snapshot_op(&m.get),
			put: snapshot_op(&m.put),
			delete: snapshot_op(&m.delete),
			delete_range: snapshot_op(&m.delete_range),
			actor_open: snapshot_op(&m.actor_open),
			actor_close: snapshot_op(&m.actor_close),
			keys_total: m.keys_total.load(std::sync::atomic::Ordering::Relaxed) as i64,
			requests_total: m.requests_total.load(std::sync::atomic::Ordering::Relaxed) as i64,
			batch_atomic_commits: m.batch_atomic_commits.load(std::sync::atomic::Ordering::Relaxed) as i64,
			batch_atomic_pages: m.batch_atomic_pages.load(std::sync::atomic::Ordering::Relaxed) as i64,
		},
		sql: SqlMetricsSnapshot {
			execute: snapshot_op(&s.execute),
			query: snapshot_op(&s.query),
			exec: snapshot_op(&s.exec),
			spawn_blocking_wait: snapshot_op(&s.spawn_blocking_wait),
			sqlite_step: snapshot_op(&s.sqlite_step),
			stmt_cache: snapshot_op(&s.stmt_cache),
			result_serialize: snapshot_op(&s.result_serialize),
		},
		vfs: VfsMetricsSnapshot {
			...
			xwrite_count: 0,
			xwrite_us: 0,
			xwrite_buffered_count: 0,
			xsync_count: 0,
			xsync_us: 0,
			commit_atomic_count: 0,
			commit_atomic_us: 0,
			commit_atomic_pages: 0,
		},
	}
}
