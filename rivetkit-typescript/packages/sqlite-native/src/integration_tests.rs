...
#[test]
fn test_truncate_produces_delete_range() {
	let rt = create_runtime();
	...
	unsafe {
		// Use truncate journal mode so COMMIT truncates the journal (xTruncate)
		// instead of deleting it (xDelete).
		...
		for i in 0..30 {
			...
			exec_sql(db.as_ptr(), &format!("INSERT INTO trunc VALUES ('{data}{i}')"));
		}
		exec_sql(db.as_ptr(), "COMMIT");
	}

	// Verify DeleteRange ops were produced.
	let ops = rt.block_on(server.ops());
	let delete_ranges: Vec<_> = ops
		.iter()
		.filter(|op| matches!(op, MockOp::DeleteRange { .. }))
		.collect();
	assert!(
		!delete_ranges.is_empty(),
		"expected DeleteRange operations from truncate"
	);

	// Verify delete ranges use correct key format.
	for op in &delete_ranges {
		if let MockOp::DeleteRange { start, end, .. } = op {
			assert_eq!(start[0], kv::SQLITE_PREFIX, "start should have SQLITE_PREFIX");
			assert_eq!(start[2], kv::CHUNK_PREFIX, "start should have CHUNK_PREFIX");
			assert_eq!(end[0], kv::SQLITE_PREFIX, "end should have SQLITE_PREFIX");
			assert_eq!(end[2], kv::CHUNK_PREFIX, "end should have CHUNK_PREFIX");
		}
	}
...
#[test]
fn test_delete_journal_produces_correct_ranges() {
	let rt = create_runtime();
	...
	unsafe {
		// Default journal_mode is "delete". After commit, the journal file is
		// removed via xDelete or xClose(DELETEONCLOSE), both of which use
		// DeleteRange for chunks.
		exec_sql(
			db.as_ptr(),
			"CREATE TABLE djtest (x TEXT)",
		);
		exec_sql(db.as_ptr(), "BEGIN");
		for i in 0..20 {
			exec_sql(
				db.as_ptr(),
				&format!("INSERT INTO djtest VALUES ('row_{i}')"),
			);
		}
		exec_sql(db.as_ptr(), "COMMIT");
	}
	...
	// Verify journal-related DeleteRange ops with correct bounds.
	let journal_delete_ranges: Vec<_> = ops
		.iter()
		.filter(|op| {
			matches!(op, MockOp::DeleteRange { start, .. }
				if start.len() >= 4
				&& start[0] == kv::SQLITE_PREFIX
				&& start[2] == kv::CHUNK_PREFIX
				&& start[3] == kv::FILE_TAG_JOURNAL)
		})
		.collect();
	assert!(
		!journal_delete_ranges.is_empty(),
		"expected DeleteRange for journal file tag after commit"
	);

	// Verify the range bounds match the expected chunk key layout.
	for op in &journal_delete_ranges {
		if let MockOp::DeleteRange { start, end, .. } = op {
			let expected_start = kv::get_chunk_key(kv::FILE_TAG_JOURNAL, 0);
			let expected_end = kv::get_chunk_key_range_end(kv::FILE_TAG_JOURNAL);
			assert_eq!(
				start.as_slice(),
				expected_start.as_slice(),
				"delete range start should be chunk_key(JOURNAL, 0)"
			);
			assert_eq!(
				end.as_slice(),
				expected_end.as_slice(),
				"delete range end should be chunk_key_range_end(JOURNAL)"
			);
		}
	}
...
fn test_file_tags_encoding() {
	...
	// Verify journal operations occurred via ops log (journal keys may be
	// deleted by the time we check the store).
	let ops = rt.block_on(server.ops());
	let journal_puts = ops.iter().any(|op| {
		matches!(op, MockOp::Put { keys, .. }
			if keys.iter().any(|k| k.len() >= 4 && k[3] == kv::FILE_TAG_JOURNAL))
	});
	assert!(
		journal_puts,
		"expected Put ops with journal file tag during transaction"
	);
