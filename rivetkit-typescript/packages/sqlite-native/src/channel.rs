...
/// Per-operation-type metrics for KV channel operations.
...
impl OpMetrics {
	...
	pub fn snapshot(&self) -> (u64, u64, u64, u64) {
		...
		(count, total, if min == u64::MAX { 0 } else { min }, max)
	}
}

/// Aggregated metrics for all KV channel operations.
pub struct KvChannelMetrics {
	pub get: OpMetrics,
	pub put: OpMetrics,
	pub delete: OpMetrics,
	pub delete_range: OpMetrics,
	pub actor_open: OpMetrics,
	pub actor_close: OpMetrics,
	pub keys_total: AtomicU64,
	pub requests_total: AtomicU64,
	pub batch_atomic_commits: AtomicU64,
	pub batch_atomic_pages: AtomicU64,
}

impl KvChannelMetrics {
	pub fn new() -> Self {
		Self {
			get: OpMetrics::new(),
			put: OpMetrics::new(),
			delete: OpMetrics::new(),
			delete_range: OpMetrics::new(),
			actor_open: OpMetrics::new(),
			actor_close: OpMetrics::new(),
			keys_total: AtomicU64::new(0),
			requests_total: AtomicU64::new(0),
			batch_atomic_commits: AtomicU64::new(0),
			batch_atomic_pages: AtomicU64::new(0),
		}
	}

	fn record_op(&self, data: &RequestData, duration: Duration) {
		match data {
			RequestData::KvGetRequest(_) => self.get.record(duration),
			RequestData::KvPutRequest(_) => self.put.record(duration),
			RequestData::KvDeleteRequest(_) => self.delete.record(duration),
			RequestData::KvDeleteRangeRequest(_) => self.delete_range.record(duration),
			RequestData::ActorOpenRequest => self.actor_open.record(duration),
			RequestData::ActorCloseRequest => self.actor_close.record(duration),
		}
	}
}
...
struct Inner {
	...
	/// Fires once the first WebSocket connection is established. Allows
	/// `send_request` to wait for the initial connection instead of
	/// returning `ConnectionClosed` immediately.
...
impl KvChannel {
	...
	pub async fn send_request(
		...
		// Wait for the initial WebSocket connection before attempting to send.
		if !self.inner.ever_connected.load(std::sync::atomic::Ordering::SeqCst) {
			match time::timeout(
				Duration::from_millis(KV_EXPIRE_MS),
				self.inner.connected_notify.notified(),
			)
			.await
			{
				Ok(()) => {}
		...
		// Save the operation type tag and key count before data is moved.
		let key_count: u64 = match &data {
			RequestData::KvGetRequest(body) => body.keys.len() as u64,
			RequestData::KvPutRequest(body) => body.keys.len() as u64,
			RequestData::KvDeleteRequest(body) => body.keys.len() as u64,
			_ => 0,
		};
		let op_tag = match &data {
			RequestData::KvGetRequest(_) => 0u8,
			RequestData::KvPutRequest(_) => 1,
			RequestData::KvDeleteRequest(_) => 2,
			RequestData::KvDeleteRangeRequest(_) => 3,
			RequestData::ActorOpenRequest => 4,
			RequestData::ActorCloseRequest => 5,
		};
		...
		let result = match time::timeout(Duration::from_millis(KV_EXPIRE_MS), resp_rx).await {
			Ok(Ok(result)) => result,
		...
		// Record round-trip duration by operation type.
		let duration = start.elapsed();
		let m = &self.inner.metrics;
		match op_tag {
			0 => m.get.record(duration),
			1 => m.put.record(duration),
			2 => m.delete.record(duration),
			3 => m.delete_range.record(duration),
			4 => m.actor_open.record(duration),
			_ => m.actor_close.record(duration),
		}
		m.keys_total.fetch_add(key_count, Ordering::Relaxed);
		m.requests_total.fetch_add(1, Ordering::Relaxed);

		result
	}

	/// Get a reference to the channel metrics.
	pub fn metrics(&self) -> &KvChannelMetrics {
		&self.inner.metrics
	}
...
async fn connection_loop(inner: Arc<Inner>, mut shutdown_rx: watch::Receiver<bool>) {
	...
	loop {
		...
		// Use tungstenite's IntoClientRequest to build the request with proper
		// headers, then add the "rivet" subprotocol. The engine's guard
		// unconditionally adds Sec-WebSocket-Protocol: rivet to upgrade
		// responses, so the client must request it to avoid a protocol error.
		let ws_request = {
			use tokio_tungstenite::tungstenite::client::IntoClientRequest;
			let mut req = url.as_str().into_client_request().expect("failed to build websocket request");
		...
		match connect_async(ws_request).await {
			Ok((ws_stream, _)) => {
				...
				// Signal waiters that the initial connection is ready.
				if !inner.ever_connected.swap(true, std::sync::atomic::Ordering::SeqCst) {
