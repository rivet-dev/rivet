...
/**
 * Internal: Workflow is waiting for signals.
 * This is thrown to yield control back to the scheduler.
 */
export class SignalWaitError extends Error {
	constructor(public readonly signalNames: string[]) {
		super(`Waiting for signals: ${signalNames.join(", ")}`);
		this.name = "SignalWaitError";
	}
}
...
export class StepExhaustedError extends Error {
	constructor(
		...
		super(`Step "${stepName}" exhausted retries: ${lastError ?? "unknown error"}`);
