...
export const KEY_PREFIX = {
	...
	SIGNALS: 3,         // Signal queue: [3, index]
...
function tupleToSegment(element: TupleItem): PathSegment {
	...
	throw new Error(`Invalid path segment tuple element: ${JSON.stringify(element)}`);
}
...
/**
 * Build a key for a signal.
 * Key: [3, signalId]
 *
 * Signal IDs can be either:
 * - A string UUID (for new signals added via handle.signal())
 * - Used with the prefix to list all signals
 */
export function buildSignalKey(signalId: string): Uint8Array {
	return pack([KEY_PREFIX.SIGNALS, signalId]);
}

/**
 * Build a prefix for listing all signals.
 * Prefix: [3]
 */
export function buildSignalPrefix(): Uint8Array {
	return pack([KEY_PREFIX.SIGNALS]);
}
...
/**
 * Parse a signal key and return the signal ID.
 * Key: [3, signalId] → signalId
 */
export function parseSignalKey(key: Uint8Array): string {
	const elements = unpack(key);
