...
function branchStatusTypeToBare(status: InternalBranchStatusType): BareBranchStatusType {
	switch (status) {
		case "pending":
...
function branchStatusTypeFromBare(status: BareBranchStatusType): InternalBranchStatusType {
	switch (status) {
	...
	switch (kind.type) {
		case "step":
			return {
				tag: "StepEntry",
				val: {
					output: kind.data.output !== undefined ? encodeCbor(kind.data.output) : null,
		...
		case "signal":
			return {
				...
				val: {
					name: kind.data.name,
					signalData: encodeCbor(kind.data.data),
				},
			};
		case "join":
			return {
				...
				val: {
					branches: new Map(
						Object.entries(kind.data.branches).map(([name, status]) => [
							name,
	...
	switch (kind.tag) {
		case "StepEntry":
			return {
				type: "step",
				data: {
					output: kind.val.output !== null ? decodeCbor(kind.val.output) : undefined,
		...
		case "SignalEntry":
			return {
				...
				data: {
					name: kind.val.name,
					data: decodeCbor(kind.val.signalData),
				},
			};
		case "JoinEntry":
			return {
				...
				data: {
					branches: Object.fromEntries(
						Array.from(kind.val.branches.entries()).map(([name, status]) => [
							name,
		...
		case "RemovedEntry":
			return {
				type: "removed",
				data: {
					originalType: kind.val.originalType as InternalEntryKind["type"],
					originalName: kind.val.originalName ?? undefined,
				},
			};
		default:
			throw new Error(`Unknown entry kind: ${(kind as { tag: string }).tag}`);
	}
}
...
export function serializeEntry(entry: InternalEntry): Uint8Array {
	...
	return ENTRY_VERSIONED.serializeWithEmbeddedVersion(bareEntry, CURRENT_VERSION);
}
...
function entryMetadataToBare(metadata: InternalEntryMetadata): v1.EntryMetadata {
	return {
		...
		completedAt: metadata.completedAt !== undefined ? BigInt(metadata.completedAt) : null,
	};
}

function entryMetadataFromBare(bareMetadata: v1.EntryMetadata): InternalEntryMetadata {
	return {
		...
		completedAt: bareMetadata.completedAt !== null ? Number(bareMetadata.completedAt) : undefined,
		dirty: false,
	};
}

export function serializeEntryMetadata(metadata: InternalEntryMetadata): Uint8Array {
	const bareMetadata = entryMetadataToBare(metadata);
	return ENTRY_METADATA_VERSIONED.serializeWithEmbeddedVersion(bareMetadata, CURRENT_VERSION);
}

export function deserializeEntryMetadata(bytes: Uint8Array): InternalEntryMetadata {
...
// === Signal Conversion & Serialization ===

function signalToBare(signal: InternalSignal): v1.Signal {
	return {
		id: signal.id,
		name: signal.name,
		signalData: encodeCbor(signal.data),
		sentAt: BigInt(signal.sentAt),
	};
}
...
function signalFromBare(bareSignal: v1.Signal): InternalSignal {
	return {
		id: bareSignal.id,
		name: bareSignal.name,
		data: decodeCbor(bareSignal.signalData),
		sentAt: Number(bareSignal.sentAt),
	};
}

export function serializeSignal(signal: InternalSignal): Uint8Array {
	const bareSignal = signalToBare(signal);
	return SIGNAL_VERSIONED.serializeWithEmbeddedVersion(bareSignal, CURRENT_VERSION);
}

export function deserializeSignal(bytes: Uint8Array): InternalSignal {
	const bareSignal = SIGNAL_VERSIONED.deserializeWithEmbeddedVersion(bytes);
	return signalFromBare(bareSignal);
}

// === Workflow Metadata Serialization ===
// Note: These are used for reading/writing individual workflow fields

export function serializeWorkflowState(state: InternalWorkflowState): Uint8Array {
...
export function deserializeWorkflowState(bytes: Uint8Array): InternalWorkflowState {
	const decoder = new TextDecoder();
	const state = decoder.decode(bytes) as InternalWorkflowState;
	const validStates: InternalWorkflowState[] = ["pending", "running", "sleeping", "failed", "completed", "cancelled"];
...
export function deserializeWorkflowOutput<T>(bytes: Uint8Array): T {
	try {
		...
		throw new Error(`Failed to deserialize workflow output: ${error instanceof Error ? error.message : String(error)}`);
	}
}
...
export function serializeWorkflowError(error: SerializedWorkflowError): Uint8Array {
	return cbor.encode(error);
}

export function deserializeWorkflowError(bytes: Uint8Array): SerializedWorkflowError {
	try {
...
export function deserializeWorkflowInput<T>(bytes: Uint8Array): T {
	try {
		...
		throw new Error(`Failed to deserialize workflow input: ${error instanceof Error ? error.message : String(error)}`);
	}
}
