...
use rivetkit_core::{
	...
	FactoryRequest, FlatActorConfig, GetWorkflowHistoryRequest, OnBeforeActionResponseRequest,
	OnBeforeConnectRequest, OnConnectRequest, OnDestroyRequest, OnDisconnectRequest,
	OnMigrateRequest, OnRequestRequest, OnSleepRequest, OnStateChangeRequest, OnWakeRequest,
	OnWebSocketRequest, ReplayWorkflowRequest, Request, Response, RunRequest,
};
...
impl CallbackBindings {
	fn from_js(callbacks: JsObject) -> napi::Result<Self> {
		let actions = if let Some(actions) = callbacks.get::<_, JsObject>("actions")? {
			...
			for name in JsObject::keys(&actions)? {
				let callback = actions.get::<_, JsFunction>(&name)?.ok_or_else(|| {
					napi::Error::from_reason(format!("action `{name}` must be a function"))
				})?;
		...
		Ok(Self {
			...
			on_disconnect: optional_tsfn(&callbacks, "onDisconnect", build_connection_payload)?,
			actions,
	...
	fn create_callbacks(&self) -> ActorInstanceCallbacks {
		...
		callbacks.on_wake =
			wrap_void_callback("onWake", &self.on_wake, |request: OnWakeRequest| {
		...
		callbacks.on_destroy = wrap_void_callback(
			"onDestroy",
			&self.on_destroy,
		...
		callbacks.run = wrap_void_callback("run", &self.run, |request: RunRequest| {
			LifecyclePayload { ctx: request.ctx }
		});
		callbacks.get_workflow_history = wrap_optional_buffer_callback(
			"getWorkflowHistory",
			&self.get_workflow_history,
			|request: GetWorkflowHistoryRequest| WorkflowHistoryPayload { ctx: request.ctx },
		);
	...
	Map: Fn(Req) -> Payload + Send + Sync + 'static,
{
	...
	Some(Box::new(move |request| {
		...
		Box::pin(async move { call_void(callback_name, &callback, (map.as_ref())(request)).await })
	}))
}
	...
	Map: Fn(OnRequestRequest) -> HttpRequestPayload + Send + Sync + 'static,
{
	...
	Some(Box::new(move |request| {
		...
		let map = Arc::clone(&map);
		Box::pin(
			async move { call_request(callback_name, &callback, (map.as_ref())(request)).await },
		)
	}))
}
	...
	Map: Fn(ActionRequest) -> ActionPayload + Send + Sync + 'static,
{
	...
	Box::new(move |request| {
		...
		Box::pin(
			async move { call_buffer(&callback_name, &callback, (map.as_ref())(request)).await },
		)
	})
}
...
fn wrap_optional_buffer_callback<Req, Payload, Map>(
	...
	Box<
		dyn Fn(
				...
				Box<dyn std::future::Future<Output = Result<Option<Vec<u8>>>> + Send + 'static>,
			> + Send
			+ Sync,
	>,
>
...
async fn call_void<T>(callback_name: &str, callback: &CallbackTsfn<T>, payload: T) -> Result<()>
where
	T: Send + 'static,
{
	let promise = callback
