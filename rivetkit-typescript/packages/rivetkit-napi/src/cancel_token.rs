...
static CANCEL_TOKENS: LazyLock<SccHashMap<u64, CancellationToken>> = LazyLock::new(SccHashMap::new);
#[cfg(test)]
static CANCEL_TOKEN_TEST_LOCK: LazyLock<Mutex<()>> = LazyLock::new(|| Mutex::new(()));
...
pub(crate) fn register_token() -> (u64, CancellationToken) {
	...
	tracing::debug!(cancel_token_id = id, "registered native cancel token");
	(id, token)
}
...
pub(crate) fn lookup_token(id: u64) -> Option<CancellationToken> {
	let token = CANCEL_TOKENS.read_sync(&id, |_, token| token.clone());
	tracing::debug!(
		cancel_token_id = id,
		found = token.is_some(),
		"looked up native cancel token"
	);
	token
}
...
pub(crate) fn cancel(id: u64) {
	if let Some(token) = CANCEL_TOKENS.read_sync(&id, |_, token| token.clone()) {
		tracing::debug!(
			cancel_token_id = id,
			"abort signal cancelled registered native cancel token"
		);
		token.cancel();
	} else {
		tracing::debug!(
			cancel_token_id = id,
			"abort signal cancel requested unknown native cancel token"
		);
	}
}
...
pub(crate) fn drop_token(id: u64) {
	...
	tracing::debug!(cancel_token_id = id, "dropped native cancel token");
}
...
#[cfg(test)]
pub(crate) fn lock_registry_for_test() -> parking_lot::MutexGuard<'static, ()> {
	CANCEL_TOKEN_TEST_LOCK.lock()
}
...
mod tests {
	use super::{
		active_token_count, cancel, drop_token, lock_registry_for_test, poll_cancelled,
		register_guarded_token, register_token,
	};
