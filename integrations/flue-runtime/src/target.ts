...
export interface RivetTargetOptions {
	/** Module exporting an `actors` object to merge into the generated registry. */
	actors?: string;
}
...
function generateEntryPoint(ctx: BuildContext, options: RivetTargetOptions): string {
	...
	const userAppImport = ctx.appEntry
		...
		: '';
	const userActorsImport = options.actors
		? `import * as userActorsModule from ${JSON.stringify(path.resolve(ctx.root, options.actors).replace(/\\/g, '/'))};`
		: 'const userActorsModule = {};';
...
const userRivetActors = userActorsModule.actors ?? userActorsModule.default ?? {};
if (!userRivetActors || typeof userRivetActors !== 'object' || Array.isArray(userRivetActors)) {
  throw new TypeError('[flue] The Rivet actors module must export an actors object or a default actor map.');
}
...
if (actorNameCollisions.length > 0) {
  throw new Error('[flue] Rivet actors module collides with generated actor names: ' + actorNameCollisions.join(', '));
}
const rivetActors = { ...userRivetActors, ...internalRivetActors };
