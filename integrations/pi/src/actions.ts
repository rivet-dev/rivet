...
export interface PiActions {
	/**
	 * Sends a prompt. Resolves once Pi accepts it; the run continues in the actor
	 * and streams through the `event` event. Use `waitForIdle` to block on it.
	 */
	prompt: (
		c: PiContext,
		text: string,
...
export function createPiActions(options: PiSessionOptions): PiActions {
	...
	return {
		prompt: (c, text, promptOptions) =>
			mutate(c, ({ session }) => startPrompt(c, session, text, promptOptions)),
...
/**
 * Starts a Pi run and resolves once Pi has accepted the prompt. The run itself
 * holds the actor awake until it finishes; its progress streams as events.
 */
	...
	return new Promise<void>((resolve, reject) => {
		...
		c.keepAwake(
			run.then(
				...
				(error: unknown) => {
					if (accepted) {
						c.log.error({ msg: "pi run failed after acceptance", error });
						return;
					}
					reject(error);
				},
