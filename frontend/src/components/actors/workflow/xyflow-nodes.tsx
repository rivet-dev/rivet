...
export function WorkflowNode({ data, selected }: NodeProps<WorkflowNodeType>) {
	...
	return (
		...
		>
			{/* Spinning border effect for running/retrying nodes */}
			{isActive && (
				<div
					className="absolute -inset-[2px] rounded-[12px] animate-spin"
					style={{
						background: `conic-gradient(from 0deg, transparent 60%, ${isRetrying ? "#f59e0b" : "#3b82f6"})`,
...
export function BranchGroupNode({
	data,
}: NodeProps<BranchGroupNodeType>) {
	const colors = TYPE_COLORS[data.entryType];
	const isFailed =
