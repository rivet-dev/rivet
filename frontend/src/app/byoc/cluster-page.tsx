...
export function ClusterPage({ cluster }: { cluster: string }) {
	...
	const regions = useAllRegions(cluster);
	const regionCount = regions.data?.length;
	const hasMoreRegions = regions.hasNextPage;
...
function useAllRegions(cluster: string) {
	const dataProvider = useCloudDataProvider();
	usePrefetchInfiniteQuery({
...
function ClusterStatus({
	...
	return (
		<>
			...
			{formatRegionCount(regionCount, hasMore)}{" "}
			{regionCount === 1 && !hasMore ? "region" : "regions"}
		</>
	);
}

function formatRegionCount(count: number, hasMore: boolean) {
...
function ResourcesCard({
	...
	return (
		<Card title="Resources">
			<ul className="mt-4 divide-y divide-foreground/10 border-t border-foreground/10 text-sm">
				<ResourceRow
					icon={faDownload}
				...
				<WithTooltip
					...
					trigger={
						<li
							className="flex"
							tabIndex={
								!isLoading && !data?.token ? 0 : undefined
							}
						>
			...
			<Card>
				<TabsList className="px-3 pt-2">
					<TabsTrigger value="regions">
						Regions
						{regionCount ? (
							<span className="ml-1.5 rounded-sm bg-foreground/[0.06] px-1.5 py-0.5 text-[11px] font-normal text-muted-foreground">
								{formatRegionCount(regionCount, hasMoreRegions)}
							</span>
						) : null}
					</TabsTrigger>
					<TabsTrigger value="commands">Commands</TabsTrigger>
					<TabsTrigger value="metrics-token">
						Metrics token
					</TabsTrigger>
				</TabsList>
				...
				<TabsContent value="metrics-token" className="mt-0 p-4">
					<OtelTokenPanel key={cluster} cluster={cluster} />
				</TabsContent>
			</Card>
		</Tabs>
	);
...
function RegionsPanel({ cluster }: { cluster: string }) {
	const { data, isLoading, isError, refetch, hasNextPage } =
		useAllRegions(cluster);
...
function RegionFilter({ cluster }: { cluster: string }) {
	...
	const { data } = useAllRegions(cluster);
...
function OtelTokenPanel({ cluster }: { cluster: string }) {
	const dataProvider = useCloudDataProvider();
	...
	return (
		<>
			<SmallText className="text-muted-foreground">
				Authorizes sending OpenTelemetry metrics from this cluster to
				Rivet. Keep it secret.
			</SmallText>
			<div className="mt-3">
				{freshToken ? (
