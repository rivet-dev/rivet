		...
		<div className="h-full border my-2 mr-2 px-4 py-4 rounded-lg flex flex-col items-center justify-safe-center overflow-auto @container">
			<div className="max-w-3xl w-full mb-6">
				<div className="border rounded-lg w-full bg-card">
					<div className="rounded-t-lg flex justify-between items-center px-6 py-4 sticky -top-4 bg-card border-b z-10">
...
function Templates() {
	...
	return (
		<motion.div className="grid grid-cols-2 gap-4 gap-y-10 my-4" variants={{hidden: {opacity: 0.8}, show: {opacity: 1}}} initial="hidden" animate="show">
				{templates
					.toSorted((a, b) => (a.priority || Number.MAX_SAFE_INTEGER) - (b.priority || Number.MAX_SAFE_INTEGER))
		...
		>
			...
			>
				...
				<div className="flex">
					...
					<Icon icon={faChevronRight} className="ml-auto mt-3 pl-4 text-muted-foreground group-hover:opacity-100 opacity-0 transition-opacity" />
				</div>
...
const MotionLinkComponent = React.forwardRef<HTMLAnchorElement, ComponentProps<typeof motion.a>>(
  (props, ref) => {
    return (
...
export function ExamplePreview({slug, title, className}: {slug: string; title: string; className?: string}) {
	return <div className={cn("rounded-xl border relative -bottom-4 w-full group-hover:-bottom-3 transition-all overflow-hidden", className)}>
		...
		<div className="flex items-center gap-2 border-b bg-muted px-3 py-2">
			...
			<Picture className="border-b min-h-40 w-full rounded-b-xl ">
				...
				<PictureImage
					...
					src={`/examples/${slug}/image.png`} alt={title}
				/>
			</Picture>
	</div>
}