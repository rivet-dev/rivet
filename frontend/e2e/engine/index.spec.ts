...
test.describe("initial load", () => {
    test("when not authenticated, should prompt user for admin token and authenticate successfully", async ({ authenticated }) => {
        await authenticated;
    });
    test("when authenticated, should redirect to the first available namespace", async ({ authenticated }) => {
        ...
        await authenticated.getByTestId(TEST_IDS.Layout.Sidebar).waitFor({ timeout: 10_000 });
